...
if (choixttt==2 | choixttt==12 | choixttt==23 | choixttt==123){
  ...
  dp3d_plot_t<-list(); dp_plot2dlist<-list()
        ...
        for (k2 in k2_choosen) { # k2=k2_choosen 
          for (t in taus_choosen){ # t= taus_choosen
            for (rep in reponse_choosen){ # rep=reponse_choosen[1]
              ...
              # 2D static graphic : RASTER
             dp2dF_plot_list[[sdmname]] <- ggplot(RqMod_mat) +
                ...
                guides(alpha = "none") +
                Scalf_rq2d() +
                facet_wrap(taust ~ .); dp2dF_plot_list[[sdmname]]
              dp_plot2dlist[[length(dp_plot2dlist)+1]] <- dp2dF_plot_list[[sdmname]]
              ggsave(sprintf("%s%s/%s/%s_%s_nlRQ22d_%s_%s_%s_%s_%s.tiff",
                     ...
                     plot = dp2dF_plot_list[[sdmname]], width = 10, height = 9, dpi=600)
              ...
              dp3d_plot <- plot_ly(showlegend=F, scene=paste("scene",rep,sep="")) %>% 
                ...
                layout(title = titleG, scene = myscene); dp3d_plot
              dp3d_plot_t[[length(dp3d_plot_t)+1]] <- dp3d_plot
              saveWidget(dp3d_plot, sprintf("%s%s/%s/%s_%s_nlRQ23d_%s_%s_%s_%s_%s.html",
                                            wdgraphEx,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],
                                            saison[sa,2],yt,x1t,x2t,taus[t]),
                         selfcontained = F, libdir = "lib")
            }
            plot_rq2dbis <- ggarrange(plotlist=dp_plot2dlist, ncol=2, nrow=1,
            ...
            dp3dF_plot_list <- subplot(dp3d_plot_t, shareX = TRUE, shareY = TRUE, nrows = 2) %>%
              layout(title = paste(titleG,subtitleG,sep="\n"),
                     scene = scene, scene2 = scene2)
            saveWidget(dp3dF_plot_list,sprintf("%s%s/%s/%s_%s_nlRQ23dFacet_%s_%s_%s_%s.html",
                                          wdgraphEx,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],
                                          saison[sa,2],x1t,x2t,taus[t]),
                       selfcontained = F, libdir = "lib")
          }
