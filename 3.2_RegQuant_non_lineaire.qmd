...
rdatain <- sprintf("%s%s_BDD.RData",wdmat,etude)
load(rdatain)
```
...
rdataout  <- sprintf("%s%s_nlRQ_BDD.RData",wdmat,etude)
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
      ...
      for (rep in answ){ # rep=1
        for (k in explo) { # k=1 
          ...
          sm_plot_list[[sdmname]] <- ggplot(smrq_t,aes(x=tau,y=Value, group = 1)) +
            ...
            labs(title = titleG,subtitle = subtitleG)
          ...
          for (k2 in explo[-c(1:k)]) { #k2=2
            ...
            for (t in 1:length(taus)){ # t=4
              ...
              dp3d_plot_t[[t]] <- plot_ly(showlegend=F, 
                                   scene=paste0("scene",t)) %>% 
                add_surface(x = gridx1x2mat[["x"]], y = gridx1x2mat[["y"]], 
                            z = RqMod_mat,
                            opacity = 0.9, 
            ...
            nnrow<-2
            nncol<-ceiling(length(taus)/nnrow)
            nly <- seq(from=0,to=1,length.out=nnrow+1)
            nlx <- seq(from=0,to=1,length.out=nncol+1)
            nlx<-matrix(rep(cbind(nlx[1:nncol],nlx[2:(nncol+1)]),nncol),
                        ncol = 2,byrow = TRUE)
            nly<-cbind(rep(nly[1:nnrow],rep(nnrow,nnrow)),
                       rep(nly[2:(nnrow+1)],rep(nnrow,nnrow)))
            
            scene1 <- list.append(myscene,domain=list(x=nlx[1,],y=nly[1,]))
            scene2 <- list.append(myscene,domain=list(x=nlx[2,],y=nly[2,]))
            scene3 <- list.append(myscene,domain=list(x=nlx[3,],y=nly[3,]))
            scene4 <- list.append(myscene,domain=list(x=nlx[4,],y=nly[4,]))
            # scene5 <- list.append(myscene,domain=list(x=nlx[5,],y=nly[5,]))
            # scene6 <- list.append(myscene,domain=list(x=nlx[6,],y=nly[6,]))
            dp3dF_plot_list[[sdmname]] <- plotly::subplot(dp3d_plot_t) %>%
              layout(scene = scene1, scene2 = scene2, 
                     scene3 = scene3, scene4 = scene4) #, scene5 = scene5, scene6 = scene6
        ...
        dp_plot <- ggarrange(plotlist = dp_plot,
                               ncol=3, nrow=length(names(dp_plot))/3,
        ...
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/nlRQ1/%s_%s_nlRQ1_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/nlRQ1/%s_%s_nlRQ1Resid_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/nlRQ2/%s_%s_nlRQ2Resid_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/nlRQ1/%s_%s_nlRQ1_sm_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/nlRQ2/%s_%s_nlRQ2_sm_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
  ...
  AIC_plot<- smrq_l %>% 
    ...
    labs(title="Quantile Regression AIC scores : Gaussian Non-linear",
