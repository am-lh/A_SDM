...
rdataout  <- sprintf(paste(wdwork,etude,"_nlRQ_BDD.RData", sep=""))
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
  modelq_nlrq_list <- list()
  RqMod_nlrq_list <- list();  smrq_t_nlrq_list <- list(); rqResid_t_nlrq_list <- list()
  rqlim_t_nlrq_list <- list(); rqinf_t_nlrq_list <- list(); rqsup_t_nlrq_list <- list();
  for (sp in spe) { # sp=1
    ...
    for (sa in sai) {# sa=1
      for (rep in answ){ # rep=1
        for (k in explo) { # k=1 
          ...
          modelq_nlrq_list[[sdmname]] <- modelq_t
          RqMod_nlrq_list[[sdmname]] <- RqMod_t  <- RqMod_t %>% map_dfr(., ~.)
          rqlim_t_nlrq_list[[sdmname]] <- rqlim_t <- rqlim_t %>% map_dfr(., ~.)
          rqsup_t_nlrq_list[[sdmname]] <- rqsup_t <- rqsup_t %>% map_dfr(., ~.)
          rqinf_t_nlrq_list[[sdmname]] <- rqinf_t <- rqinf_t %>% map_dfr(., ~.)
          smrq_t_nlrq_list[[sdmname]] <- smrq_t <- smrq_t %>% map_dfr(., ~.)
          rqResid_t_nlrq_list[[sdmname]] <- rqResid_t <- rqResid_t %>% map_dfr(., ~.)
          ...
          for (t in 1:length(taus)){ # required to have diff color for rq lines
            modelq <- modelq_nlrq_list[[sdmname]][[t]]
            plot_dp <- plot_dp + 
  ...
  smrq_l <- smrq_t_nlrq_list %>% map_dfr(., ~.)
        ...
        for (k2 in k2_choosen) { # k2=k2_choosen 
          for (t in taus_choosen){ # t= taus_choosen
            for (rep in reponse_choosen){ # rep=reponse_choosen[1]
              ...
              modelq <- modelq_nlrq_list[[sdmname_chosen]]
              smrq_t <- smrq_t_nlrq_list[[sdmname_chosen]]
              RqMod_t <- RqMod_nlrq_list[[sdmname_chosen]]
              rqlim_t <- rqlim_t_nlrq_list[[sdmname_chosen]]
              rqsup_t <- rqsup_t_nlrq_list[[sdmname_chosen]]
              rqinf_t <- rqinf_t_nlrq_list[[sdmname_chosen]]
              df <- CSLN_Mars[which(CSLN_Mars$SPCourt == speciesMP$SPCourt[sp]),]
              yt = reponse[rep,1] ; yl = sprintf("%s (%s)",reponse[rep,2],reponse[rep,3])
              zt = "Zone"
              x1t = sprintf("%s%s",pred_red[k,1],saison[sa,1]); x1l = sprintf("%s%s (%s)",pred_red[k,2],saison[sa,1],pred_red[k,3])
             ...
             plot_dp2d <- ggplot(RqMod_mat) +
              ...
              ggsave(sprintf("%s%s/%s/%s_%s_nlRQ22d_%s_%s_%s_%s_%s.tiff",
                             wdgraph,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],
            ...
            ggsave(sprintf("%s%s/%s/%s_%s_nlRQ22d_%s_%s_%s_%s.tiff",
                           wdgraph,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],
