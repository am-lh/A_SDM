...
# TREATMENT OF BIOLOGIC DATA FROM CSLN_mud_l BEFORE SDM TREATMENT
# Amelie LEHUEN Janvier 2022
#_______________________________________________________________________________
# library(diffr)
# setwd("C:/Users/lehuen201/Nextcloud/Melting Pot/BDD/A_SDM_NEO/Scripts")
# diffr("1.1 Community Analysis Zone.R","1.2 Community Analysis Station.R")
...
library(broom); library(purrr) # Multiple lm at once : tidy,glance,augment
# Graphics packages
...
# If exists
# load(file = paste(wdwork,"CSLN_Mars_Zone_BDD",".RData", sep=""))
...
#________________________________________________________________
# SELECTION OF DATA (ONLY MUDFLAT done previously) FOR GIPSA ----
CSLN_mud_l<-CSLN_mud %>%
  select(idStationUnique,Station_originelle,Zone,Period,Tidal_level,Annee,SP,SPCourt,Taxon_SNa,Biomass_gAFDWm2,Density_indm2,MSRtot,BPc)
...
# MSR
aov_MSR <- aov(MSRtot ~ Period * Zone, data = CSLN_mud_Spch)
...
BTA_cont_names <- subset(CSLN_mud_BTA_names, rowSums(BTA_cont) != 0) #
BTA_cont <- subset(BTA_cont, rowSums(BTA_cont) != 0) #
MSR_ado <-adonis2(BTA_cont ~ BTA_cont_names$Period * BTA_cont_names$Zone, method = "bray", permutations = 999)
...
MSR_padsig <- as.data.frame(rbind(padZ, padP, padZP)) %>% filter(sig != "")

# BPc
aov_BPc <- aov(BPc ~ Period * Zone, data = CSLN_mud_Spch) #
...
BTA_cont_names <- subset(CSLN_mud_BTA_names, rowSums(BTA_cont) != 0) #
BTA_cont <- subset(BTA_cont, rowSums(BTA_cont) != 0) #
BPc_ado <-adonis2(BTA_cont ~ BTA_cont_names$Period * BTA_cont_names$Zone, method = "bray", permutations = 999)
padZ <- pairwise.adonis(BTA_cont, BTA_cont_names$Zone)
padP <- pairwise.adonis(BTA_cont, BTA_cont_names$Period)
padZP <- pairwise.adonis(BTA_cont, BTA_cont_names$ZP)
...
# ANCOVA BTA----
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# MIEUX DEFINIR LES UNITES DU CALCUL BPc ET SON USAGE
#  + DIFF DE RESULTAT AVEC MAXIME
...
gp <-ggplot(CSLN_mud_Spch,aes(y = BPc, x = MSRtot, colour = Taxon_SNa)) + 
  geom_point() +
  geom_smooth(method = "lm") + 
  stat_regline_equation(aes(label =  paste(..eq.label.., ..rr.label.., sep = "~~~~"),color = Taxon_SNa)) +
  Scale_brew()+
...
# number of 0 present in the database for a given speciesMP, but also the rare 
# speciesMP (absent from many stations) but abundant for some stations.
# A coefficient f, between 0 and 1, allows to adjust the weight given to the 
# frequency of null values for a speciesMP (f=0 if only this criterion is taken .
# into account) and to the abundance of speciesMP expressed in log
...
# INDVAL ----
  ...
  # abundance of each speciesMP and its occurrence in the samples to define an index 
  # between 0 and 1. The higher the IndVal, the more characteristic the speciesMP 
...
# OUTPUT SAVE ----
  ...
  save.image(file = paste(wdwork,"CSLN_Mars_Zone_BDD",".RData", sep=""))
