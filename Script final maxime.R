...
#  brewer.pal() est utilis?e pour g?n?rer un vecteur de couleurs BARPLOT
...
CSLN.data <-
  as.data.frame(read_excel(fauna_file, sheet = "Biology_station", na = ""))
granulo <-
  as.data.frame(read_excel(fauna_file, sheet = "Granulo", na = ""))
...
CSLN <- CSLN.data %>% left_join(granulo[, -c(21:24)]) %>%
  dplyr::arrange(idStationUnique, Taxon) %>% dplyr::mutate(IndBodySize_gAFDW =
                                                             Biomass_gAFDWm2 /
                                                             Density_indm2)#calcul de la densite INTERMUD en fonction de l'abondance
CSLN$IndBodySize_gAFDW[CSLN$Density_indm2 == 0] <- 0

# ajout  Biomasse sur Intermud
tmp <- CSLN %>% filter(Filtre == "INTERMUD") %>%
  select(ScientificName_accepted) %>% left_join(
    CSLN %>% group_by(ScientificName_accepted) %>%
      summarise(Mediane.IndBodySize = median
                (IndBodySize_gAFDW, na.rm = T))
  ) %>%
  select(Mediane.IndBodySize)

CSLN$Biomass_gAFDWm2[CSLN$Filtre == "INTERMUD"] <-
  as.vector(tmp$Mediane.IndBodySize)

# Mise en forme longitude latitude
CSLN <- CSLN %>%
  filter(
    longitude != "NaN",
    latitude != "NaN",!is.na(Density_indm2),
    !is.na(Biomass_gAFDWm2),
    !is.na(AphiaID_accepted),
...
CSLN$specix[CSLN$ScientificName_accepted %in% c(
  "Eteone longa",
  "Crangon crangon",
  "Oligochaeta",
  "Carcinus maenas",
...
CSLN <- CSLN %>% filter(!grepl("d?bris", ScientificName_accepted))
...
#Donn?es GEOgraphique----

CSLN.geo <-
  CSLN  %>% st_as_sf(coords = c("longitude", "latitude"), #%>% filter(Zone=="North Downstream Mudflat", "North Median Mudflat", "North Upstream Mudflat", "South Mudflat")
...
station_uniq <-
  CSLN %>% filter(grepl("Mudlfat", Zone)) %>% distinct(Station_originelle)


#GIS Integration Mars dans les donn?es biologiques

Mars.csv <-
  read.csv("C:/Users/maxcb/OneDrive/Bureau/StageM1/BDD/Resultats/ES_Mars_Maps_sh.csv")
Mars.csv[Mars.csv == "NaN"] <- NA
Mars.shp <-
  Mars.csv %>% filter(!is.na(Lon), !is.na(Lat), !is.na(flow_m)) %>%
  st_as_sf(coords = c("Lon", "Lat"), crs = 4326) %>% st_transform(2154)
CSLN.geo$NINJ <-
  Mars.shp$NINJ[st_nearest_feature(CSLN.geo, Mars.shp)] #ajoute le NINJ au CSV

#Integration de la zone dans les donn?es biologiques
Gis.shp <-
  read_sf("C:/Users/maxcb/OneDrive/Bureau/StageM1/SIG/Layers_made/ES_Areas_WGS.shp")
Gis.shp <-
  Gis.shp %>%  st_transform(2154) %>% filter(
    Zone != "NA" & Zone != "Bay"  &
      Zone != "OffShore" &
      Zone != "Octeville" &
      Zone != "Cote Fleurie" &
...
# Intersection entre zone et coordonn?e des points de pr?l?vements pour identifier dans quelle zone sont les points
CSLN.geo <-
  CSLN.geo %>% mutate(
    intersection  = as.integer(st_intersects(geometry, Gis.shp)),
    Zone = if_else(is.na(intersection), "NA", Gis.shp$Zone[intersection]),
    Type = if_else(is.na(intersection), "NA", Gis.shp$Type[intersection])
  )
CSLN.geo$Zone[CSLN.geo$Zone == "NA"] <-
  NA
CSLN.geo$Type[CSLN.geo$Type == "NA"] <-
  NA #trnasformation du champs NA en donn?e manquante
CSLN.geo <- CSLN.geo %>% filter(
  Zone != "NA" & Zone != "Bay"  &
    Zone != "OffShore" &
    Zone != "Octeville" &
    Zone != "Cote Fleurie" &
...
#r?aliser une jointure entre le Mars.CSV et forme 2 qui ont un champs en commun le NINJ et l'ann?e
...
tm_Bio <-
  tm_basemap("OpenStreetMap.HOT") + # OpenStreetMap.HOT .Mapnik .France - Stamen.Watercolor #https://leaflet-extras.github.io/leaflet-providers/preview/
...
for (i in which(!is.na(CSLN$temp_m) &
                CSLN$specix == "Les especes +++")) {
  CSLN$MSR_mW[i] <- msr(
    CSLN$Density_indm2[i],
    CSLN$Biomass_gAFDWm2[i],
    CSLN$AphiaID_accepted[i],
    CSLN$temp_m[i],
    1,
    "w"
  ) * 1000 #Mean MSR (mW/ind)
}
...
Tb.MSR2 <-
  Tb.MSR1 %>% group_by(idStationUnique, Zone, Annee, periode)  %>% summarise(MSR.an.station = sum(MSRtot, na.rm =
                                                                                                   T))

Tb.MSR <-
  Tb.MSR2 %>% group_by(Annee, periode, Zone) %>% summarise(MSR = mean(MSR.an.station, na.rm =
                                                                        T)) %>% filter(periode != "2000-2005")

library(ggplot2)
graph.msr <-
  ggplot(Tb.MSR) + geom_boxplot(aes(y = MSR, fill = periode)) + coord_cartesian(ylim = c(0, 100)) +
  facet_grid(. ~ Zone, scales = "free_y") + labs(title = "(A)", y = "MSR (mW/m?)") +
  theme(axis.text.x = element_text(size = 0))
graph.msr

# Potentiel de bioturbation (BPc & quieros)
potentiel.de.bioturbation <-
  read.csv("C:/Users/maxcb/OneDrive/Bureau/potentiel de bioturbation.csv",
           sep = ";")
CSLN <- CSLN %>% left_join(potentiel.de.bioturbation)
CSLN$BPc <-
  sqrt(CSLN$Biomass_gAFDWm2 / CSLN$Density_indm2) * CSLN$Density_indm2 * CSLN$Mi *
  CSLN$Ri
...
pbiot <-
  CSLN %>% dplyr::select(
    ...
    Mi,
    specix
  ) %>% filter(specix == "Les especes +++", Zone != "Channel")
pbiot$BPc <-
  sqrt(pbiot$Biomass_gAFDWm2 / pbiot$Density_indm2) * pbiot$Density_indm2 *
  pbiot$Mi * pbiot$Ri
Tb.pBiot <-
  pbiot %>% group_by(idStationUnique, Annee, periode, Zone) %>% summarise(pbiot.an.station = sum(BPc, na.rm =
                                                                                                   T))
Tb.pBiot2 <-
  Tb.pBiot %>% group_by(Annee, periode, Zone) %>% summarise(Potentiel_de_bioturbation =
                                                              mean(pbiot.an.station, na.rm = T))


graph.pbiot <-
  ggplot(Tb.pBiot2) + geom_boxplot(aes(y = Potentiel_de_bioturbation, fill = periode)) + #coord_cartesian( ylim = c(0, 100)) +
  facet_grid(. ~ Zone, scales = "free_y") + labs(title = "(B)", y = "Potentiel de bioturbation") + theme(axis.text.x = element_text(size =
                                                                                                                                      0))
graph.pbiot
...
CSLN.BTA <-
  CSLN %>% filter(
    periode != "2000-2005",
    Zone != "Channel",
    ScientificName_accepted == "Hediste diversicolor" |
      ScientificName_accepted == "Scrobicularia plana" |
      ScientificName_accepted == "Macoma balthica" |
      ScientificName_accepted == "Cerastoderma edule" |
      ScientificName_accepted == "Peringia ulvae" |
      ScientificName_accepted == "Corophium volutator",
    !is.na(MSRtot),
    !is.na(BPc),
    BPc < 8000,
    MSRtot < 75
...
#MSR
a.MSR <- aov(MSRtot ~ periode * Zone, data = CSLN.BTA) #Tb.MSR
shapiro.test(a.MSR$residuals)
ggqqplot(residuals(a.MSR))
bartlett.test(MSRtot ~ periode, data = CSLN.BTA)
bartlett.test(MSR ~ Zone, data = Tb.MSR)
msr_con <- as.matrix(Tb.MSR[, -c(1:3)])
row.names(msr_con) <-
  paste(Tb.MSR$periode, Tb.MSR$Annee, Tb.MSR$Zone, sep = "_")

# a.BP <- aov(BPc~periode*Zone, data = CSLN.BTA)
# shapiro.test(a.BP$residuals)
# ggqqplot(residuals(a.MSR))
# bartlett.test(BPc~periode, data = CSLN.BTA)
# bartlett.test(BPc~Zone, data = CSLN.BTA)
...
perm <- adonis2(Tb.MSR ~ Zone)
pairwise.adonis(Tb.MSR, Tb.MSR$periode)

djk <-
  adonis2(
    msr_con ~ msr_con_names$periode * msr_con_names$Zone,
    #*CSLN_cont.mat_names$Annee,
    method = "bray",
    permutations = 999
  )
...
MSR_tabAD <-
  as.data.frame(rbind(MSR_padZ, MSR_padP, MSR_padZP)) %>% filter(sig != "")
...
BP_con <- as.matrix(Tb.pBiot2[, -c(1:3)])
row.names(BP_con) <-
  paste(Tb.pBiot2$periode, Tb.pBiot2$Annee, Tb.pBiot2$Zone, sep = "_")
...
bp.adon <-
  adonis2(
    BP_con ~ BP_con_names$periode * BP_con_names$Zone,
    #*CSLN_cont.mat_names$Annee,
    method = "bray",
    permutations = 999
  )
...
#ANCOVA BTA----
regr <-
  ggplot(CSLN.BTA) + geom_point(aes(x = MSRtot, y = BPc, colour = ScientificName_accepted))
regr #, na.rm = T
Table.MSR.sp <-
  CSLN.BTA %>% filter(ScientificName_accepted == "Peringia ulvae")
cor.test(Table.MSR.sp$MSRtot, Table.MSR.sp$BPc)


hgv <-
  lm(CSLN.BTA$BPc ~ CSLN.BTA$MSRtot * CSLN.BTA$ScientificName_accepted)
graph <-
  ggplot(CSLN.BTA,
         aes(y = BPc, x = MSRtot, colour = ScientificName_accepted)) + geom_point() +
  geom_smooth(method = "lm") + scale_color_brewer(palette = "Set1")
graph
graph1 <-
  graph + stat_regline_equation(aes(
    label =  paste(..eq.label.., ..rr.label.., sep = "~~~~"),
    color = ScientificName_accepted
  )) +
  labs(y = "Potentiel de bioturbation", x = "MSR (mW/m?)", color = "Espece")
graph1 #R? de 0,90

# # Calculer le mod?le, la covariable passe en premier
# model<-lm(CSLN.BTA$BPc ~ CSLN.BTA$MSRtot + CSLN.BTA$ScientificName_accepted)
# # Inspecter les param?tres de diagnostic du mod?le
# model.metrics <- augment(model)  # Supprimer les d?tails
# head(model.metrics, 3)
# # ?valuer la normalit? des r?sidus ? l'aide du test de Shapiro-Wilk
...
# kruskal.test(BPc ~ ScientificName_accepted, data = CSLN.BTA)
#
#
# pairwise.wilcox.test(CSLN.BTA$BPc~CSLN.BTA$MSRtot, CSLN.BTA$ScientificName_accepted)
#
# sm.ancova(CSLN.BTA$MSRtot, CSLN.BTA$BPc, CSLN.BTA$ScientificName_accepted)
# CSLN.BTA<-as.vector(CSLN.BTA$MSRtot)
# CSLN.BTA.NUM <- CSLN.BTA %>% filter (Zone=="North Downstream Mudflat")
# regr2<- ggplot(CSLN.BTA.NUM) + geom_point(aes(x=MSRtot, y=BPc)); regr2
# regrrr<- ggplot() + geom_point(aes(x=Tb.MSR$MSR.moy, y =Tb.pBiot2$pbiot.moy));regrrr
#
#
# regrelin<- CSLN %>% dplyr::select(Annee,periode, Zone, ScientificName_accepted, Density_indm2,specix, MSRtot, BPc) %>% filter(Zone!="Channel",specix == "Les especes +++")
# reg_lin <- lm(MSRtot~BPc, data=regrelin)
# summary(regrelin)

Fauna_Brey.spe <-
  read.csv("C:/Users/maxcb/OneDrive/Bureau/StageM1/Scripts/MSR/Fauna_Brey spe.csv",
           sep = ";")
...
#Indice par Ann?e/Periode ####AJOUTER niv.tidal a chaque select,group, filter
...
sttaion <- CSLN_mud %>% distinct(SPCourt, ScientificName_accepted)

CSLN_mud$periode <- as.factor(CSLN_mud$periode)
...
CSLN_mud_spm <-
  CSLN_mud %>% select(
    ...
    Biomass_gAFDWm2,
    specix
  ) %>% # filter(specix=="Les 6 especes") %>%
  complete(
    nesting(Zone, Annee, periode),
    nesting(ScientificName_accepted),
    fill = list(Biomass_gAFDWm2 = 0, Density_indm2 = 0)
  ) %>%
  group_by(Zone, Annee, periode, ScientificName_accepted) %>%
  summarise(
    Biomasse.moy = mean(Biomass_gAFDWm2, na.rm = TRUE),
...
OCC.mudflat <-
  CSLN_mud %>% dplyr::select(
    ...
    Biomass_gAFDWm2,
    specix
  ) %>% count(specix, SPCourt, ScientificName_accepted) %>%
...
OCC.mudflat.grapho <-
  OCC.mudflat %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
  geom_text(aes(y = 0.5, label = Pourcentage)) + #
  labs(title = "(A)", x = "esp?ce", y = "Occurrence (%)") + #
...
OCC.mudflatDensity$Densite_moyenne <-
  round(OCC.mudflatDensity$Densite_moyenne, digits = 1)

OCC.mudflat.graphD <-
  OCC.mudflatDensity %>% #pour avoir la table lancer avant pipe
  ggplot2::ggplot(aes(
    x = reorder(ScientificName_accepted, Densite_moyenne),
    y = Densite_moyenne
  )) + geom_col(fill  = "grey", colour = "grey") +
  geom_text(aes(y = 50, label = Densite_moyenne)) + # Xlab("Densit? (ind/m?)") #
  labs(title = "(B)", x = "esp?ce", y = "Densit? moyenne (ind/m?)") + #
...
OCC.mudflatbiom$biom_moyenne <-
  round(OCC.mudflatbiom$biom_moyenne, digits = 1)

OCC.mudflat.graphB <-
  OCC.mudflatbiom %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, biom_moyenne),
    y = biom_moyenne
  )) + geom_col(fill  = "grey", colour = "grey") +
  geom_text(aes(y = 0.25, label = biom_moyenne)) + #
  labs(title = "(C)", x = "esp?ce", y = "Biomasse moyenne (g.mati?re s?che sans cendre/m?)") + #
...
# BIOMASSE
# graph<-ggplot(CSLN_mud_spm)+ geom_boxplot(aes(x=periode, y=Biomasse.moy, fill= periode))+ ylim=200 +
#   facet_grid( .~Zone,scales = "free_y") +  theme(axis.text.x = element_text(angle = -90,size=4)) ;graph  #Attention ? l'echelle
# dev.print(device = png, file= paste(wdgraph,"CSLN_Biomasse par zone et annee, Specix.png",
#                                     sep = ""),width=1000,height = 600)
# #
# graph<-ggplot(CSLN_mud_spm)+ geom_boxplot(aes(x=periode, y=Biomasse.moy, fill= periode)) +
#   facet_grid(SPCourt~Zone,scales = "free_y") +  theme(axis.text.x = element_text(angle = -90,size=4));graph  #Attention ? l'echelle
# dev.print(device = png, file= paste(wdgraph,"CSLN_Biomasse par zone et periode, specix.png",
#                                     sep = ""),width=1000, height = 600)
#     #niv.tidal
# graph<-ggplot(CSLN_mud_spm)+ geom_boxplot(aes(x=Annee, y=Biomasse.moy, fill= Annee)) +
#   facet_grid(SPCourt~Zone+niv.tidal,scales = "free_y") ;graph  #Attention ? l'echelle
...
#   theme(axis.text.x = element_text(angle = -90,size=4));graph  #Attention ? l'echelle
# dev.print(device = png, file= paste(wdgraph,"CSLN_Biomasse par zone,niveau tidal et periode, specix.png",
#                                     sep = ""),width=1000, height = 600)

#DENSITE

# graph<-ggplot(CSLN_mud_spm)+ geom_boxplot(aes(x=Annee, y=Densite.moy, fill= Annee)) +
#   facet_grid(SPCourt~Zone,scales = "free_y") +  theme(axis.text.x = element_text(angle = -90,size=4)) ;graph  #Attention ? l'echelle
# dev.print(device = png, file= paste(wdgraph,"CSLN_Densite par zone et annee, specix.png",
#                                     sep = ""),width=1000,height = 600)

graph <-
  ggplot(CSLN_mud_spm) + geom_col(aes(x = periode, y = Densite.moy, fill = periode)) +
  facet_grid(. ~ Zone, scales = "free_y")  +  theme(axis.text.x = element_text(angle = -90, size =
                                                                                 4))
graph  #Attention ? l'echelle
#dev.print(device = png, file= paste(wdgraph,"CSLN_Densite moy  Mudflat et periode.png",
# sep = ""),width=1000, height = 600)

graph <-
  ggplot(CSLN_mud) + geom_col(aes(x = ScientificName_accepted, y = Density_indm2, fill = periode)) +
  facet_grid(. ~ Zone, scales = "free_y") +  theme(axis.text.x = element_text(angle = -90, size =
                                                                                6))
graph
#niv.tidal
...
# graph<-ggplot(CSLN_mud_spm)+ geom_boxplot(aes(x=periode, y=Densite.moy, fill= periode)) +
#   facet_grid(SPCourt~Zone+niv.tidal,scales = "free_y")+
#   theme(axis.text.x = element_text(angle = -90,size=4));graph  #Attention ? l'echelle
...
#Indices Ecologiques (feuille d'entr?e)----

#Par Zone
CSLN_Mars_sm <-
  CSLN_mud %>% group_by(Annee, periode, Zone) %>%  #niv.tidal
  summarise(
    Richesse.spe = n_distinct(ScientificName_accepted),
    n.indiv = n(),
...
#niv.tidal
# Indice.ecoZNT<-CSLN_mud %>% group_by(Annee,periode, Zone, niv.tidal) %>%  #niv.tidal
...
#PAR ANNEE et Zone #
graph <-
  ggplot(CSLN_Mars_sm) + geom_col(aes (x = Annee, y = Richesse.spe, fill =
                                        Annee)) +
  facet_grid(. ~ Zone) + #~niv.tidal , margins = T
  theme(axis.text.x = element_text(angle = -90, size = 4)) labs(title =
                                                                  "Densit?s des esp?ces sur les zones vasi?res")
graph #,scales="free_y"
# dev.print(device = png, file= paste(wdgraph,"CSLN_RS par Mediane Mudflat et annee.png",
#                                    sep = ""),width=1000, height = 600)
CSLN_Mars_sm$Annee <- as.factor(CSLN_Mars_sm$Annee)

graph <-
  ggplot(CSLN_Mars_sm) + geom_col(aes(x = Annee, y = Biomasse.sum.ZNT, fill =
                                       Annee)) +
...
#PAR PERIODE et zone
graph <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes (x = periode, y = Richesse.spe, fill =
                                            periode)) +
  facet_grid(. ~ Zone)
graph #,scales="free_y"
...
graph <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes(x = Annee, y = Biomasse.sum.ZNT, fill =
                                           Annee)) +
  facet_grid(. ~ Zone)
graph #, scales="free"
...
CSLN_cont <-
  CSLN_mud_spm %>% dplyr::select(Zone, Annee, periode, ScientificName_accepted , Densite.moy) %>% #niv.tidal
  group_by(Zone, Annee, periode, ScientificName_accepted) %>%
...
row.names(CSLN_cont.mat) <-
  paste(CSLN_cont$periode, CSLN_cont$Annee, CSLN_cont$Zone, sep = "_")
...
Indice.ecoZm <-
  CSLN_Mars_sm %>% group_by(Zone) %>% summarise(moy.Rs = mean(Richesse.spe))
class(CSLN_Mars_sm$Richesse.spe)
CSLN_Mars_sm$Richesse.spe <- as.numeric(CSLN_Mars_sm$Richesse.spe)
CSLN_Mars_sm$Shannon <- diversity(CSLN_cont.mat, index = "shannon")

CSLN_Mars_sm$Pielou <-
  CSLN_Mars_sm$Shannon / log2(CSLN_Mars_sm$Richesse.spe)
CSLN_Mars_sm$Diversite.simpson <-
  diversity(CSLN_cont.mat, index = "simpson")
CSLN_Mars_sm$E_Simpson <- (1 - CSLN_Mars_sm$Diversite.simpson - min
                          (1 - CSLN_Mars_sm$Diversite.simpson)) /
  (max(1 - CSLN_Mars_sm$Diversite.simpson) - min(1 - CSLN_Mars_sm$Diversite.simpson)) #?quitabililit? de Simpson not? E
CSLN_Mars_sm$D_Hill <-
  diversity(CSLN_cont.mat, index = "invsimpson") / exp(CSLN_Mars_sm$Shannon)

#write.xlsx(CSLN_Mars_sm, wdgraph, asTable = FALSE, overwrite = TRUE)

#graphique des indices pour le niveau tidal #Indice.ecoZNT + facet_grid(Zone~niv.tial)
#ligne184 <- RS
devtools::install_github('thomasp85/gganimate')
library(gganimate)
means <- aggregate(Densite_moyenne ~  periode, CSLN_Mars_sm, mean)
graphDens <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes (x = periode, y = Densite_moyenne, fill =
                                            periode)) + coord_cartesian(ylim = c(0, 1035)) +
  facet_grid(. ~ Zone) + xlab(" ") + ylab("Densit? moyenne (ind/m?)") + #labs(title="Richesses sp?cifiques par zone et p?riode") + #~niv.tidal , margins = T
  theme(axis.text.x = element_text(angle = -90, size = 0)) + transition_time(periode) +
  labs(title = "Year: {frame_time}")
graphDens

graphrs <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes (y = Richesse.spe, fill = periode)) +
  facet_grid(. ~ Zone) + ylab("Richesse sp?cifique") + #labs(title="Richesses sp?cifiques par zone et p?riode") + #~niv.tidal , margins = T
  theme(axis.text.x = element_text(angle = -90, size = 0))
graphrs #,scales="free_y"
# dev.print(device = png, file= paste(wdgraph,"CSLN_RS par Mediane Mudflat et annee.png",
#                                    sep = ""),width=1000, height = 600)

GraphS <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes(y = Shannon, fill = periode)) + #periode
  facet_grid(. ~ Zone) + ylab("Indice de Shannon") + #labs(title="Shannon par zone et p?riode") +
  theme(axis.text.x = element_text(size = 0))
GraphS
# dev.print(device = png, file= paste(wdgraph,"CSLN_Shannon  par p?riode et zone.png",
# sep = ""),width=1000, height = 600)

GraphP <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes(y = Pielou, fill = periode)) + #periode
  facet_grid(. ~ Zone) + ylab("Indice d'?quitabilit? Pielou") + #labs(title="Pielou par zone et p?riode") +
  theme(axis.text.x = element_text(size = 0))
GraphP
# dev.print(device = png, file= paste(wdgraph,"CSLN_Pielou par zone et p?riode.png",
# sep = ""),width=1000, height = 600)
...
# Graph<-ggplot(CSLN_Mars_sm)+geom_col(aes(x=Annee,y=Diversite.simpson,fill=Annee))+ #periode
#   facet_grid(.~Zone) + theme(axis.text.x=element_text(angle=-90, vjust=0.4)); Graph
# dev.print(device = png, file= paste(wdgraph,"CSLN_div_Simpson par zone et p?riode.png",
#                                     sep = ""),width=1000, height = 600)
#
# Graph<-ggplot(CSLN_Mars_sm)+geom_boxplot(aes(x=periode,y=E_Simpson,fill=periode))+ #periode
#   facet_grid(.~Zone) + theme(axis.text.x=element_text(angle=-90, vjust=0.4)); Graph
# dev.print(device = png, file= paste(wdgraph,"CSLN_E_Simpson par zone et p?riode.png",
#                                     sep = ""),width=1000, height = 600)
...
#sp rare----
CSLN.abd <-
  abund(CSLN_cont.mat, f = 0.4) #Application de la m?thode TPA
summary(CSLN.abd)
plot(
  CSLN.abd,
  dpos = c(40, 100),
  xlab = "Genres",
  ylab = "Abondances relatives",
  main = "M?thode TPA f=0.4"
) #Repr?sentation graphique
CSLN.abd$n <-
  30 # identify(CSLN.abd) #Un petit plateau est visible, cliquer sur la fin du plateau Number of variables extracted: 129 on a total of 616
...
CSLN_adonis <-
  adonis2(
    CSLN.TPA ~ CSLN.TPA_names$periode * CSLN.TPA_names$Zone,
    #*CSLN_cont.mat_names$Annee,
    method = "bray",
    permutations = 999
  ) #CSLN_cont.mat_names$niv.tidal
...
CSLN_tabAD <-
  as.data.frame(rbind(CSLN_padZ, CSLN_padP, CSLN_padZP)) %>% filter(sig !=
                                                                      "")
...
CSLN_cont.per <-
  CSLN %>% dplyr::select(periode,
                         Zone,
                         ScientificName_accepted,
                         Density_indm2,
                         Biomass_gAFDWm2,
                         specix) %>% #, MSR_mW, MSRtot
  filter(grepl("Mudflat", Zone)) %>% ###Changer pour autre zone
  ...
  dplyr::select(Zone, periode, ScientificName_accepted, Densite.moy) %>% #niv.tidal
  group_by(Zone, periode, ScientificName_accepted) %>%
  pivot_wider(names_from = ScientificName_accepted,values_from =Densite.moy,values_fill = 0)

CSLN_cont.mat.per <- as.matrix(CSLN_cont.per[, -c(1:2)])
row.names(CSLN_cont.mat.per) <-
  paste(CSLN_cont.per$periode, CSLN_cont.per$Zone, sep = "_")


CSLN_cont.mat.per <-
  subset(CSLN_cont.mat.per, rowSums(CSLN_cont.mat.per) != 0) #
CSLN_cont.mat_names.per <-
  data.frame(Names = row.names(CSLN_cont.mat.per)) %>%
...
#sp rare par periode
CSLN.abd.per <-
  abund(CSLN_cont.mat.per, f = 0.4) #Application de la m?thode TPA
...
CSLN.abd.per$n <-
  60 # identify(CSLN.abd) #Un petit plateau est visible, cliquer sur la fin du plateau Number of variables extracted: 129 on a total of 616
colnames(extract(CSLN.abd.per, CSLN.abd.per$n))

CSLN.TPA.per <- pastecs::extract(CSLN.abd.per, CSLN.abd.per$n)


# medianglob<-apply(CSLN_cont.mat.per,2,median) #Vecteur avec les m?dianes par genre
# boxplot (medianglob, xlab="Mediane globale", ylab="Abondances (ind/L)")
# subset(t(CSLN_cont.mat.per),medianglob>median(medianglob))->phyto2 #S?lection des genres dont les m?dians sont sup?rieurs ? la m?diane des m?dianes
# dim(phyto2)
# phytosimp<-t(phyto2)



row.names(CSLN.TPA.per) <-
  paste(CSLN_cont.per$periode, CSLN_cont.per$Zone, sep = "_")
...
Indice.ecoZP <-
  CSLN_mud.per %>% group_by(periode, Zone) %>%  #niv.tidal
  summarise(Richesse.spe = n_distinct(ScientificName_accepted),
            n.indiv = n())

Indice.ecoZP$Shannon <- diversity(CSLN.TPA.per, index = "shannon")

Indice.ecoZP$Equi.pielou <-
  Indice.ecoZP$Shannon / log2(Indice.ecoZP$Richesse.spe)
# CSLN_Mars_sm$Diversite.simpson<- diversity(CSLN_cont.mat, index = "simpson")
# CSLN_Mars_sm$E_Simpson<-(1 - CSLN_Mars_sm$Diversite.simpson - min
#                         (1 - CSLN_Mars_sm$Diversite.simpson )) /
#   ( max(1 - CSLN_Mars_sm$Diversite.simpson) - min( 1 - CSLN_Mars_sm$Diversite.simpson)) #?quitabililit? de Simpson not? E
# CSLN_Mars_sm$D_Hill<-diversity(CSLN_cont.mat,index = "invsimpson")/exp(CSLN_Mars_sm$Shannon)

#PERMANOVA : ADONIS2 et PAIRWISE
CSLN_adonis <-
  adonis2(
    CSLN_cont.mat ~ CSLN_cont.mat_names$periode * CSLN_cont.mat_names$Zone,
    #*CSLN_cont.mat_names$Annee,
    method = "bray",
    permutations = 999
  ) #CSLN_cont.mat_names$niv.tidal
...
CSLN_padP <-
  pairwise.adonis(CSLN_cont.mat, CSLN_cont.mat_names$periode)
# CSLN_padA<-pairwise.adonis(CSLN_cont.mat,CSLN_cont.mat_names$Annee)
#CSLN_padNT<-pairwise.adonis(CSLN_cont.mat,CSLN_cont.mat_names$niv.tidal)
...
# CSLN_padZNTPA<-pairwise.adonis(CSLN_cont.mat,CSLN_cont.mat_names$ZNTPA)
CSLN_tabAD <-
  as.data.frame(rbind(CSLN_padZ, CSLN_padP)) %>% filter(sig != "")


#CAH station / zone
simprof.Bray <- simprof(
  data = CSLN_cont.mat.per,
  num.expected = 1000,
  num.simulated = 999,
...
SS <-
  simprof.plot(simprof.Bray, leaflab = "perpendicular", siglinetype = 1,)
...
groupesdend <-
  simprof.Bray$significantclusters %>% set_names(seq_along(.)) %>%
  enframe %>% unnest(cols = c(value)) %>% rename(Groupe_simprof = name, code =
                                                   value) %>%
...
CSLN.ckd <-
  CSLN %>% dplyr::select(Zone, periode, ScientificName_accepted, Density_indm2, specix) %>%
  group_by(Zone, periode, ScientificName_accepted, specix) %>% summarise(d.moy = mean(Density_indm2)) %>% filter(Zone !=
                                                                                                                   "Channel", specix == "Les especes +++")

graphndm <-
  ggplot(CSLN_Mars_sm) + geom_boxplot(aes (y = Densite_moyenne, fill = periode)) + coord_cartesian(ylim = c(0, 1035)) +
  facet_grid(. ~ Zone) + #labs(title="Richesses sp?cifiques par zone et p?riode") + #~niv.tidal , margins = T
  theme(axis.text.x = element_text(angle = -90, size = 0))
graphDens
...
dev.print(
  device = png,
  file = paste(wdgraph, "CSLN_Simprof3.png", sep = ""),
  width = 1000,
  height = 600
)
dev.off()
...
#         margin=c(4,4), ylab="genres (moyennes ponder?es par sites)", xlab="Stations")
...
#CSLN_Mars_sm$groupe.CAH[<-simprof.Bray[["significantclusters"]]
...
# plot(CAH.bray,hang=-1, main="Liens complets", xlab="Zone & niveau tidal", ylab="Dissimilarit?s de Bray-Curtis")
# Choix.groupe<-simprof.Bray$hclust
# rect.hclust(CAH.bray,k=Choix.groupe)
# grp.CAH<-cutree(CAH.bray,k=Choix.groupe)
#
# CSLN_Mars_sm$grp.CAH<- grp.CAH # %>% relocate(Choix.groupe, .before = Richesse.spe) !!!!!!!!!!#NE SUIT PAS LES DONNEES SIGNIF DE SIMPROF
...
OCC.com1$Pourcentage <-
  (OCC.com1$Densitemoc / sum(OCC.com1$Densitemoc)) * 100
...
OCC.com1.graph <-
  OCC.com12 %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
...
OCC.com2 <-
  CSLN %>%  filter(Zone == "North Downstream Mudflat",
                   periode == "2006-2010",
                   specix == "Les especes +++") %>% group_by(ScientificName_accepted) %>%
  summarise(Densitemoc = mean(Density_indm2)) %>% arrange(desc(Densitemoc))
OCC.com2$Pourcentage <-
  (OCC.com2$Densitemoc / sum(OCC.com2$Densitemoc)) * 100
OCC.com2$Pourcentage <- round(OCC.com2$Pourcentage, digits = 1)
OCC.com2 <- OCC.com2 %>% dplyr::slice(1:15)
OCC.com2.graph <-
  OCC.com2 %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
...
OCC.com3 <-
  CSLN %>%  filter(Zone == "North Upstream Mudflat", specix == "Les especes +++") %>% group_by(ScientificName_accepted) %>%
  summarise(Densitemoc = mean(Density_indm2)) %>% arrange(desc(Densitemoc))
OCC.com3$Pourcentage <-
  (OCC.com3$Densitemoc / sum(OCC.com3$Densitemoc)) * 100
OCC.com3$Pourcentage <- round(OCC.com3$Pourcentage, digits = 1)
OCC.com3 <- OCC.com3 %>% dplyr::slice(1:15)
OCC.com3.graph <-
  OCC.com3 %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
...
OCC.com4 <-
  CSLN %>%  filter(Zone == "North Median Mudflat", specix == "Les especes +++") %>% group_by(ScientificName_accepted) %>%
  summarise(Densitemoc = mean(Density_indm2)) %>% arrange(desc(Densitemoc))
OCC.com4$Pourcentage <-
  (OCC.com4$Densitemoc / sum(OCC.com4$Densitemoc)) * 100
OCC.com4$Pourcentage <- round(OCC.com4$Pourcentage, digits = 1)
OCC.com4 <- OCC.com4 %>% dplyr::slice(1:15)
OCC.com4.graph <-
  OCC.com4 %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
...
OCC.com5 <-
  CSLN %>%  filter(Zone == "South Mudflat") %>% group_by(ScientificName_accepted) %>%
  summarise(Densitemoc = mean(Density_indm2)) %>% arrange(desc(Densitemoc))
OCC.com5$Pourcentage <-
  (OCC.com5$Densitemoc / sum(OCC.com5$Densitemoc)) * 100
OCC.com5$Pourcentage <- round(OCC.com5$Pourcentage, digits = 1)
OCC.com5 <- OCC.com5 %>% dplyr::slice(1:15)
OCC.com5.graph <-
  OCC.com5 %>% #pour avoir la table lancer avant pipe
  ggplot(aes(
    x = reorder(ScientificName_accepted, Pourcentage),
    y = Pourcentage
  )) + geom_col(fill = "grey", colour = "grey") +
...
# Indval ----
#### la consid?ration d'une esp?ce indicatrice comme une esp?ce sp?cifique ? un groupe donn? et fid?le ? toutes les stations/dates de ce groupe (IndVal
CSLN_cont.mat.per2 <-
  CSLN_cont.mat.per[, (!apply(CSLN_cont.mat.per == 0, 2, all))]
CSLN.Indval <- indval(CSLN.TPA.per, grp.CAH) #CSLN_cont.mat.per2
GR <-
  CSLN.Indval$maxcls #Groupe de station o? chaque genre a un indval max *
gr <-
  CSLN.Indval$indcls #Groupe de station avec indval correspondant aux indval max

pv.indval <- CSLN.Indval$pval #p-value correspondantes
tab.CSLN.Indval <-
  data.frame(group = GR,
             indval = gr,
             pvalue = pv.indval) #Tableau avec pour les esp?ces indicatrices, leur appartenance aux groupes et la p-value correspondante
tab.CSLN.Indval <-
  cbind(sp = row.names(tab.CSLN.Indval), tab.CSLN.Indval)[order(tab.CSLN.Indval$group, tab.CSLN.Indval$pvalue) , ] #Trier les indVal selon la p-value obtenue par les tests de permutation

tab.CSLN.Indval$specix <- "Autre especes"
tab.CSLN.Indval$specix[tab.CSLN.Indval[, 1] %in% c(
  "CERED",
  "CORVO",
  "HEDDI",
  "MACBA",
...
write.xlsx(tab.CSLN.Indval,
           wdgraph,
           asTable = F,
           overwrite = TRUE)

#SIMPER----
#d?terminer les contributions de chaque esp?ce ? la dissimilarit? entre stations

Simp <-
  simper(
    CSLN_cont.mat.per,
    CSLN_mds_dat$Groupe_Simprof ,
    permutations = 0,
    trace = FALSE,
...
CSLN_mds_dat <-
  data.frame(scores(CSLN.mds)) %>% # Using the scores function from vegan to extract the code scores
...
CSLN_mds_species <-
  cbind(sp = rownames(CSLN_mds_species), CSLN_mds_species)  # create a column of species, from the rownames of CSLN_mds_species
...
bp <- ggplot(data = CSLN_mds_dat) +
  ...
  annotate(
    "text",
    x = min(CSLN_mds_dat$NMDS1),
    y = min(CSLN_mds_dat$NMDS2),
    hjust = -2,
    label = paste("Stress =", round(CSLN.mds$stress, 4)),
    size = 4
  )  +
...
bp
ggsave(
  paste(wdgraph, "nMDS Macrofaune by Zone3", ".png", sep = ""),
  plot = bp,
  width = 12,
  height = 8
)
...
Sumry <-
  CSLN %>% filter(grepl("Mudflat", Zone))  %>% group_by(Zone, SPCourt, periode) %>%
  summarise(Smry = sum(Density_indm2, na.rm = TRUE), Nb = n())

Sumry$label <- paste(Sumry$SPCourt, Sumry$Nb, sep = "\n")
png(
  filename = paste(wdgraph, "_TreeMap3 Zone Density", ".png", sep = ""),
  width = 1000,
  height = 600
)
treemp <-
  treemap(
    ...
    fontcolor.labels = c("black", "white", "gray81"),
