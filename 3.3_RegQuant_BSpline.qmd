...
rdataout  <- sprintf("%sCSLN_Mars_RQbs_BDD.RData",wdwork)
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
  modelq_rqbs_list <- list()
  smrq_t_rqbs_list <- list(); RqMod_rqbs_list <- list(); # rqResid_t_rqbs_list <- list()
  rqlim_t_rqbs_list <- list(); rqinf_t_rqbs_list <- list(); rqsup_t_rqbs_list <- list();
  for (sp in spe) { # sp=1
      ...
      for (rep in answ){ # rep=1
        for (k in explo) { # k=1
          ...
          tryCatch({
            ...
            modelq_rqbs_list[[sdmname]] <- modelq
            ...
            smrq_t_rqbs_list[[sdmname]] <- smrq_t <- smrq_t %>% map_dfr(., ~.)
            ...
            RqMod_t <- melt(RqMod_t,id.vars =c("x1"),variable.name = "tau",value.name = "RqMod") %>% 
              mutate(taust=sprintf("tau= %s",tau),tau=as.numeric(as.character(tau)))
            RqMod_rqbs_list[[sdmname]] <- RqMod_t
            rqlim_t_rqbs_list[[sdmname]] <- rqlim_t
            rqsup_t_rqbs_list[[sdmname]] <- rqsup_t
            rqinf_t_rqbs_list[[sdmname]] <- rqinf_t
  ...
  smrq_l <- smrq_t_rqbs_list %>% map_dfr(., ~.)
      ...
      for (k2 in k2_choosen) { # k2=k2_choosen 
        for (t in taus_choosen){ # t= taus_choosen
          for (rep in reponse_choosen){ # rep=reponse_choosen[1]
            ...
            modelq <- modelq_rqbs_list[[sdmname_chosen]]
            smrq_t <- smrq_t_rqbs_list[[sdmname_chosen]]
            RqMod_t <- RqMod_rqbs_list[[sdmname_chosen]]
            rqlim_t <- rqlim_t_rqbs_list[[sdmname_chosen]]
            rqsup_t <- rqsup_t_rqbs_list[[sdmname_chosen]]
            rqinf_t <- rqinf_t_rqbs_list[[sdmname_chosen]]
            df <- CSLN_Mars[which(CSLN_Mars$SPCourt == speciesMP$SPCourt[sp]),]
            yt = reponse[rep,1] ; yl = sprintf("%s (%s)",reponse[rep,2],reponse[rep,3])
            zt = "Zone"
            x1t = sprintf("%s%s",pred_red[k,1],saison[sa,1]); x1l = sprintf("%s%s (%s)",pred_red[k,2],saison[sa,1],pred_red[k,3])
