...
rdataout  <- sprintf("%s%s_RQbs_BDD.RData",wdwork,etude)
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
        ...
        for (k in explo) { # k=1
          ...
          tryCatch({
            ...
            dp_plot_list[[sdmname]] <- ggplot(dfrq, aes(x=x1, y=y)) + 
              ...
              Scalc_misc() +
              theme(legend.position="bottom") +
              labs(title=titleG,x=paste(x1l," (",pred_red[k,3],")",sep=""),
                   y=yl,color=zt)
          ...
          for (k2 in explo[-c(1:k)]) { #k2=2
            ...
            # RQ calculation with error management ----
            tryCatch({
              modelq<-rq(y~bs(x1*x2,degree=3,knots=c(median(x1),median(x2))), tau=taus)
  ...
  dp_plot_list[[sdmname]]<-smrq_l %>% 
    mutate(model_desc=str_remove(predict,"B-Spline deg 3")) %>% #filter(!str_detect(mode, "4var")) %>%
    group_by(tau) %>%
