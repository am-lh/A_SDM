...
dp<-CSLN_Mars %>% count(SP,SPCourt,Taxon_SNa)%>%group_by(SP)%>%
...
ggsave(sprintf("%s%s_Species_Occr_List20.png",wdgraph,prgm), plot = dp, width = 10, height = 8)


# Treemap 2 levels : Tidal levels distribution ----
titre = paste("Total Density (ind.m-2) of Species without Offshore & Octeville"," - ", prgm,sep="")
Sumry<-CSLN_Mars %>% group_by(Tidal_level,SPCourt) %>% filter(Zone!="OffShore" & Zone!="Octeville" ) %>%
  summarise(Smry=sum(Density_indm2,na.rm=TRUE),Nb=n()) #n_distinct(idStationUnique)
Sumry$label <- paste(Sumry$SPCourt, Sumry$Nb, sep = "\n")
png(filename=paste(wdgraph,prgm,"_TreeMap2 Tidal Level Density",".png",sep=""),width=1000, height=600)
...
titre = paste("Total Biomass (gAFDW.m-2) of Species without Offshore & Octeville "," - ", prgm,sep="")
Sumry<-CSLN_Mars %>% group_by(Tidal_level,SPCourt) %>% filter(Zone!="OffShore" & Zone!="Octeville" ) %>%
  summarise(Smry=sum(Biomass_gAFDWm2,na.rm=TRUE),Nb=n()) #n_distinct(idStationUnique)
Sumry$label <- paste(Sumry$SPCourt, Sumry$Nb, sep = "\n")
png(filename=paste(wdgraph,prgm,"_TreeMap2 Tidal Level Biomass",".png",sep=""),width=1000, height=600)
treemap(Sumry,index=c("Tidal_level","label"),vSize="Smry",vColor="Tidal_level",type="index",
        title=titre, palette=colDarj(CSLN_unique$Annee), fontface.labels=c(2,2),
        border.col=c("black","white"), border.lwds=c(3,1),bg.labels=0,
        fontsize.labels=c(12,9), fontcolor.labels=c("black","white"),
...
titre = paste("Total Density (ind.m-2) of Species without Offshore & Octeville "," - ", prgm,sep="")
Sumry<-CSLN_Mars %>% group_by(Zone,Tidal_level,SPCourt) %>% filter(Zone!="OffShore" & Zone!="Octeville" ) %>%
  summarise(Smry=sum(Density_indm2,na.rm=TRUE),Nb=n())
Sumry$label <- paste(Sumry$SPCourt, Sumry$Nb, sep = "\n")
png(filename=paste(wdgraph,prgm,"_TreeMap3 Zone Density",".png",sep=""),width=1000, height=600)
...
bargraph_sd <- function(df,x,y,sd,fil,face,xlab,ylab,title,palette){
  ...
  bp <-ggplot(df, aes(x=x, y=y, fill=fil))+
    ...
    geom_errorbar(aes(ymin=max(y-sd,0), ymax=y+sd),
...
bp<-boxplot_amlh(CSLN_Mars_sm,CSLN_Mars_sm$Annee,CSLN_Mars_sm$SR,CSLN_Mars_sm$Period,CSLN_Mars_sm$Zone,
                 "Years","Specific Richness",titre,colDarj(CSLN_unique$Annee)) ;bp 
...
bp<-boxplot_amlh(CSLN_Mars_sm,CSLN_Mars_sm$Annee,CSLN_Mars_sm$Pielou,CSLN_Mars_sm$Period,CSLN_Mars_sm$Zone,
                 "Years","Index of Equitability",titre,colDarj(CSLN_unique$Annee)) ;bp 
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
...
bp<-boxplot_amlh(CSLN_Mars_sm,CSLN_Mars_sm$Annee,CSLN_Mars_sm$Biomass_t,CSLN_Mars_sm$Period,CSLN_Mars_sm$Zone,
                 "Years","Total Biomass (gAFDW/m²)",titre,colDarj(CSLN_unique$Annee))  +
                  ylim(0,750);bp 
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
...
bp<-boxplot_amlh(CSLN_Mars_sm,CSLN_Mars_sm$Annee,CSLN_Mars_sm$MSRtot_t,CSLN_Mars_sm$Period,CSLN_Mars_sm$Zone,
                 "Years","Total MSRtot (mW/m²)",titre,colDarj(CSLN_unique$Annee)) ;bp 
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
...
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 15, height = 15)

#________________________________________________________________
## Evolution temporelle par zone ----
# df  <-  CSLN_Mars_sm %>% unite(AS,Annee,Season,sep = "_", remove = FALSE)
df  <-  CSLN_Mars_sm %>% group_by(Zone,Period,Annee) %>% 
...
bp <-bargraph_sd(df,df$Annee,df$SR_m,df$SR_sd,df$Period,df$Zone,
...
bp <-bargraph_sd(df,df$Annee,df$Biomass_t_m,df$Biomass_t_sd,df$Period,df$Zone,
                 xlab,"Total Biomass (gAFDW/m²)",titre,colDarj(CSLN_unique$Zone)) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.4,hjust=1)); bp
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
bp1<-bp+ labs(title="Total Biomass (gAFDW/m²)",y="") + theme(legend.position='none')

titre <- paste("Temporal Evolution of Pielou index",sep="")
bp <-bargraph_sd(df,df$Annee,df$Pielou_m,df$Pielou_sd,df$Period,df$Zone,
                 xlab,"Index of Equitability",titre,colDarj(CSLN_unique$Zone)) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.4,hjust=1)); bp
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
bp1<-bp+ labs(title="Index of Equitability",y="") + theme(legend.position='none')

titre <- paste("Temporal Evolution of metabolic rate",sep="")
bp <-bargraph_sd(df,df$Annee,df$MSRtot_t_m,df$MSRtot_t_sd,df$Period,df$Zone,
                 xlab,"Total MSRtot (mW/m²)",titre,colDarj(CSLN_unique$Zone)) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.4,hjust=1)); bp
ggsave(paste(wdgraph,"Graph ",titre,".png",sep=""), plot = bp, width = 18, height = 6)
bp1<-bp+ labs(title="Total MSRtot (mW/m²)",y="") + theme(legend.position='none')
...
bp<-ggplot(CSLN_Mars_sp)+geom_boxplot(aes(x=Period,y=Biomass_gAFDWm2,fill=Period))+ 
...
for (sp in spe){ #sp=1
  ...
  for (rep in answ){ #rep=1
    ...
    z <- "Period"
    titreG <- paste(y," time evolution for ",species[sp,2], " - ", prgm,sep="")
    bp <- ggplot(df, aes_string(x,y,fill=z)) + geom_boxplot() + 
      ...
      Scale_fill(CSLN_unique$Annee) #bp
    ggsave(paste(wdgraph,species[sp,1],"/",etude,"_Bxp_Zone_",species[sp,1],"_",y,".png",sep=""), plot = bp, width = 12, height = 6)
  ...
  if (length(answ)>1 ){
    titreG <- paste("Responses time evolution for ",species[sp,2], " - ", prgm,sep="")
    ...
    bp <- annotate_figure(bp, top = text_grob(titreG, face = "bold", size = 14)); print(bp)
...
for (sp in spe){ 
  df <- CSLN_Mars_sp[which(CSLN_Mars_sp$SPCourt == species[sp,1]),]
  titreG <- paste("Time evolution for ",species[sp,2], " - ", prgm,sep="")
  bp <- ggplot(df, aes(Annee,Density_indm2,fill=Period)) + geom_boxplot() + 
    ...
    theme(axis.text.x=element_text(angle=-90, vjust=0.4,hjust=1)) +
    Scale_fill(CSLN_unique$Annee); bp
  ggsave(paste(wdgraph,species[sp,1],"/",etude,"_Bxp1_",species[sp,1],"_",y,".png",sep=""), plot = bp, width = 12, height = 6)
}
