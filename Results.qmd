...
library(sf) ; library(sfheaders); library(rnaturalearth) #library(raster)
library(tmap); library(tmaptools); library(OpenStreetMap) # tmap_mode; for static and interactive maps
...
france <- ne_states(country = "France", returnclass = "sf") %>%
  filter(type_en %in% c("Metropolitan department")) %>% 
  filter(!region %in% c("Corse"))
# boxestuary=c(xmin=-0.1, ymin=49.3, xmax=0.45, ymax=49.65)
estuary <- ne_states(country = "France", returnclass = "sf") %>%
...
df <- ES_Areas %>% 
  ...
  st_transform(4326)
df_stations <- CSLN_sf %>% 
  dplyr::summarize(unique(idStationUnique))
osm_df <- read_osm(df, ext=1.1)
boxestuary<-bb(df)
tm_france<-
  tm_shape(france) + tm_fill() +
  tm_shape(st_as_sfc(boxestuary)) + tm_borders("red", lwd = 2)
...
tm_Bio
print(tm_france, vp = grid::viewport(0.7, 0.8, width = 0.3, height = 0.3))
# tmap_save(tm_Bio, filename = "Estuary_map.png")
