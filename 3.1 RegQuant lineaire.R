...
add <- function(x1,x2,a,b,c){a + b*x1 + c*x2;}
inter <- function(x1,x2,a,b,c,d) {a + b*x1 + c*x2 + d*x1*x2}

# Definition des SDM choisis
name<-c('SDM1','SDM2','SDM3','SDM4')
SDM_desc<-c('Linear two factors','Linear two factors','Linear two factors','Linear two factors')
type<-c('*','*','*','*')
SDM_tau<-c(0.975,0.975,0.975,0.95)
yt<-c('Biomass_gAFDWm2','Biomass_gAFDWm2','Biomass_gAFDWm2','Biomass_gAFDWm2')
x1t<-c('flow_mxd','flow_mxd','inunt','tenfon_m')
x2t<-c('inunt','mudrate_m','mudrate_m','mudrate_m') #c('inunt','mudrate_m','sal_m','sal_m')
# SDM_x3<-c('','','','mudrate_m')
sdmlist<-data.frame(name,SDM_desc,type,SDM_tau,yt,x1t,x2t) #,SDM_x3
sdm<-1:nrow(sdmlist)
# ----
# if exists
    ...
    for (sa in sai) {# sa=1
      ...
      for (k in explo) { # k=1 
        ...
        dp <-  ggplot(dfrq, aes_string(x=x1t, y=yt, color=zt)) + geom_point() + coldiscS(nbcol) +
          ...
          for (t in 1:length(taus)){
          dp <- dp + geom_quantile(aes_string(x1t,yt),quantiles = taus[t],method = "rq",
                                   colour = colorsD(length(taus))[t], size = 0.8, alpha = 0.8)+
        ...
        ggsave(sprintf("%s%s/RQ Lineaire/RQL Simple/%s_Rq_%s_%s_%s_%s.png",
        ...
        for (k2 in explo[-c(1:k)]) { #k2=2
          ...
          tryCatch({
            modelq<-rq(y~x1*x2, tau=taus)
            modelq0<-rq(y ~ 1, tau=taus)
            tmp <- c(species[sp,1],yl,xl,"Interaction2",
                     round(median(1 - modelq$rho/modelq0$rho),5),
                     round(median(AIC(modelq)),1))
            indrq_tmp <- rbind(indrq_tmp,data.frame(t(tmp))); tmp<-NA
            smrq <- summary(modelq, se="boot")
            for (t in 1:length(taus)){
              tmp<-data.frame(Sp=species[sp,1],reponse=yl,predict=xl,mode="Interaction2",tau=taus[t],
                              Var=xxx,as.data.frame(smrq[[t]][["coefficients"]])) #modelq[["coefficients"]][,t]
              smrq_tmp <- rbind(smrq_tmp,tmp); tmp<-NA
            }
            smrq<-NULL; modelq<-NULL        
        ...
        for (k2 in explo[-c(1:k)]) {
          for (k3 in explo[-c(1:k2)]) { #k3=4
            ...
            xt<-paste(x1t,x2t,x3t,sep='+'); xl<-paste(x1l,x2l,x3l,sep='+')
            xxx <-c("Beta 0",x1l,x2l,x3l)
            dfrq <-df[,c(yt,x1t,x2t,x3t)]; dfrq <-as.data.frame(na.omit(dfrq))
            y <- jitter(dfrq[,1]); x1 <- jitter(dfrq[,2]); x2 <- jitter(dfrq[,3]); x3 <- jitter(dfrq[,4]);
            tryCatch({
              modelq<-rq(y~x1+x2+x3, tau=taus)
              modelq0<-rq(y ~ 1, tau=taus)
              tmp <- c(species[sp,1],yl,xl,"Addition3",
                       round(median(1 - modelq$rho/modelq0$rho),5),
                       round(median(AIC(modelq)),1))
              indrq_tmp <- rbind(indrq_tmp,data.frame(t(tmp))); tmp<-NA
              smrq <- summary(modelq, se="boot")
              for (t in 1:length(taus)){
                tmp<-data.frame(Sp=species[sp,1],reponse=yl,predict=xl,mode="Addition3",tau=taus[t],
                                Var=xxx,as.data.frame(smrq[[t]][["coefficients"]])) #modelq[["coefficients"]][,t]
                smrq_tmp <- rbind(smrq_tmp,tmp); tmp<-NA
              }
              smrq<-NULL; modelq<-NULL        
            ...
            xt<-paste(x1t,x2t,x3t,sep='*'); xl<-paste(x1l,x2l,x3l,sep='*')
            xxx <-c("Beta 0",x1l,x2l,x3l,paste(x1l,x2l,sep=':'),paste(x1l,x3l,sep=':'),paste(x2l,x3l,sep=':'),paste(x1l,x2l,x3l,sep=':'))
            dfrq <-df[,c(yt,x1t,x2t,x3t)]; dfrq <-as.data.frame(na.omit(dfrq))
            y <- jitter(dfrq[,1]); x1 <- jitter(dfrq[,2]); x2 <- jitter(dfrq[,3]); x3 <- jitter(dfrq[,4]);
            tryCatch({
              modelq<-rq(y~x1*x2*x3, tau=taus)
              modelq0<-rq(y ~ 1, tau=taus)
              tmp <- c(species[sp,1],yl,xl,"Interaction3",
                       round(median(1 - modelq$rho/modelq0$rho),5),
                       round(median(AIC(modelq)),1))
              indrq_tmp <- rbind(indrq_tmp,data.frame(t(tmp))); tmp<-NA
              smrq <- summary(modelq, se="boot")
              for (t in 1:length(taus)){
                tmp<-data.frame(Sp=species[sp,1],reponse=yl,predict=xl,mode="Interaction3",tau=taus[t],
                                Var=xxx,as.data.frame(smrq[[t]][["coefficients"]])) #modelq[["coefficients"]][,t]
                smrq_tmp <- rbind(smrq_tmp,tmp); tmp<-NA
              }
              smrq<-NULL; modelq<-NULL        
        ...
        for (k2 in explo[-c(1:k)]) {
          for (k3 in explo[-c(1:k2)]) {
            for (k4 in explo[-c(1:k3)]) { #k4=4
              ...
              xt<-paste(x1t,x2t,x3t,x4t,sep='+'); xl<-paste(x1l,x2l,x3l,x4l,sep='+')
              xxx <-c("Beta 0",x1l,x2l,x3l,x4l)
              dfrq <-df[,c(yt,x1t,x2t,x3t,x4t)]; dfrq <-as.data.frame(na.omit(dfrq))
              y <- jitter(dfrq[,1]); x1 <- jitter(dfrq[,2]); x2 <- jitter(dfrq[,3]); x3 <- jitter(dfrq[,4]); x4 <- jitter(jitter(dfrq[,5]));
              tryCatch({
                modelq<-rq(y~x1+x2+x3+x4, tau=taus)
                modelq0<-rq(y ~ 1, tau=taus)
                tmp <- c(species[sp,1],yl,xl,"Addition4",
                         round(median(1 - modelq$rho/modelq0$rho),5),
                         round(median(AIC(modelq)),1))
                indrq_tmp <- rbind(indrq_tmp,data.frame(t(tmp))); tmp<-NA
                smrq <- summary(modelq, se="boot")
                for (t in 1:length(taus)){
                  tmp<-data.frame(Sp=species[sp,1],reponse=yl,predict=xl,mode="Addition4",tau=taus[t],
                                  Var=xxx,as.data.frame(smrq[[t]][["coefficients"]])) #modelq[["coefficients"]][,t]
                  smrq_tmp <- rbind(smrq_tmp,tmp); tmp<-NA
                }
                smrq<-NULL; modelq<-NULL
              ...
              xt<-paste(x1t,x2t,x3t,x4t,sep='*'); xl<-paste(x1l,x2l,x3l,x4l,sep='*')
              dfrq <-df[,c(yt,x1t,x2t,x3t,x4t)]; dfrq <-as.data.frame(na.omit(dfrq))
              y <- jitter(dfrq[,1]); x1 <- jitter(dfrq[,2]); x2 <- jitter(dfrq[,3]); x3 <- jitter(dfrq[,4]); x4 <- jitter(dfrq[,5]);
              tryCatch({
                modelq<-rq(y~x1*x2*x3*x4, tau=taus)
                xxx <-rownames(coef(modelq))
                modelq0<-rq(y ~ 1, tau=taus)
                tmp <- c(species[sp,1],yl,xl,"Interaction4",
                         round(median(1 - modelq$rho/modelq0$rho),5),
                         round(median(AIC(modelq)),1))
                indrq_tmp <- rbind(indrq_tmp,data.frame(t(tmp))); tmp<-NA
                smrq <- summary(modelq, se="boot")
                for (t in 1:length(taus)){
                  tmp<-data.frame(Sp=species[sp,1],reponse=yl,predict=xl,mode="Interaction4",tau=taus[t],
                                  Var=xxx,as.data.frame(smrq[[t]][["coefficients"]])) #modelq[["coefficients"]][,t]
                  smrq_tmp <- rbind(smrq_tmp,tmp); tmp<-NA
                }
                smrq<-NULL; modelq<-NULL
...
dp<-indrq_l %>% group_by(Sp) %>% filter(!str_detect(mode, "4") & !str_detect(mode, "3")) %>% filter(!str_detect(mode, "Add")) %>% 
  ...
  labs(title="Linear Quantile Regression AICm scores", x="Model",y="AICm") +
  theme(axis.text = element_text(size=15,face="bold")) +
  coord_flip()+theme_bw();dp
ggsave(sprintf("%s%s/RQ Lineaire/%s_Rq_%s_AIC_scores.png",wdgraph,species[sp,1],prgm,species[sp,1]), plot = dp, width = 8, height = 8)
# dev.off()

# Sauvegarde des outputs ----
wb <- loadWorkbook(paste(wdres,prgm,"_BDD",".xlsx", sep="")) # addWorksheet(wb, sheetName = "rql")
writeData(wb, sheet = "Rq", x = indrq_l, startCol = 1, startRow = 1,withFilter = FALSE)
...
for (sp in spe) { #sp=1
  ...
  for (sdi in sdm) { #sdi=1
    for (sa in sai) {# sa=1
      ...
      ggsave(sprintf("%s%s/RQ Lineaire/Summaries/%s_Rq_sm_%s_%s_%s_%s.png",
      ...
      for (t in 1:length(taus)){ # t=6 length(taus)
        tryCatch({
          ...
          RqMod[[t]]<-coef(modelq)[1] + coef(modelq)[2]*grid[["x"]] + coef(modelq)[3]*grid[["y"]] +
                        coef(modelq)[4]*grid[["x"]]*grid[["y"]]
          RqMod[[t]][which(RqMod[[t]]<0)]<-NA
          
          # 2D static graphic : RASTER
          dp2d <- ggplot() +
            geom_raster(aes(x = grid[["x"]], y = grid[["y"]], fill = RqMod[[t]])) +
          ...
          ggsave(sprintf("%s%s/RQ Lineaire/%s_Rq2d_%s_%s_%s_%s_%s_%s.png",
          ...
          dp3 <- dp1 %>% add_surface(x = grid[["x"]], y = grid[["y"]], z = RqMod[[t]],
                                     opacity = 0.7,
                        ...
                        layout(title = titreG)
          htmlwidgets::saveWidget(dp3,sprintf("%s%s/RQ Lineaire/Graphes 3D/%s_Rq2d_%s_%s_%s_%s_%s_%s.html",
          ...
          dp4 <- dp4 %>% add_surface(x = grid[["x"]], y = grid[["y"]], z = RqMod[[t]],
                                     opacity = 0.5, colorscale = list(c(0,1),c(blank,collist[t])),
                                     colorbar=list(title=list(text=paste("SDM-NEO\n",reponse[rep,3],"\nTau=",taus[t],sep=""))))# %>%
          
          # 3D static graphic
          # data conversion
          # zmod<-RqMod[[t]]; rownames(zmod) = x1; colnames(zmod) = x2
          ...
          # persp(x = grid[["x"]], y = grid[["y"]], z = RqMod[[t]], col = color, phi = 30, theta = -30)
      ...
      dp4 <- dp4 %>% add_trace(x = x1sup, y = x2sup, z = ysup, mode = "markers", type = "scatter3d",
                               marker = list(size = 2, color = prettyred, symbol = 104)) 
      htmlwidgets::saveWidget(dp4, sprintf("%s%s/RQ Lineaire/Graphes 3D/%s_Rq2dStack_%s_%s_%s_%s_%s.html",
  ...
  st_write(Mars_SDM, sprintf("%sLayers made/SDM_NEO_rq_%s.shp",wdGIS,species[sp,1]),append=FALSE)
}#spe
  # Sauvegarde des outputs ----
  wb <- loadWorkbook(paste(wdres,prgm,"_BDD",".xlsx", sep="")) # addWorksheet(wb, sheetName = "RQ_line")
  writeData(wb, sheet = "Rq_sdmlist", x = sdmlist, startCol = 1, startRow = 1,withFilter = FALSE)
  saveWorkbook(wb,file=paste(wdres,prgm,"_BDD",".xlsx", sep=""), overwrite = TRUE)
  save.image(file = paste(wdwork,etude,"_RQ_BDD",".RData", sep=""))
} #end if choixttt 2
# ----
