...
rdataout  <- sprintf("%s%s_RQ_BDD.RData",wdmat,etude)
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
  ...
  for (sp in spe) { # sp=1
    ...
    for (sa in sai) {# sa=1
      for (rep in answ){ # rep=1
        for (k in explo) { # k=1 
          ...
          for (k2 in explo[-c(1:k)]) { #k2=2
            ...
            tryCatch({
              ...
              ggsave(sprintf(
                "%s%s/%s/RQ2add/%s_%s_RQ2add2d_%s_%s_%s_%s.tiff",
                             wdgraph,speciesMP$SPCourt[sp],
                             analysis,etude,speciesMP$SPCourt[sp],
                             saison[sa,2],yt,x1t,x2t),
            ...
            tryCatch({
              ...
              dp2dF_plot_list[[sdmname]] <- ggplot(RqMod_t) +
                ...
                facet_wrap(taust ~ .); print(dp2dF_plot_list[[sdmname]])
              ggsave(sprintf(
                "%s%s/%s/RQ2int/%s_%s_RQ2int2d_%s_%s_%s_%s.tiff",
                             wdgraph,speciesMP$SPCourt[sp],
                             analysis,etude,speciesMP$SPCourt[sp],
                             saison[sa,2],yt,x1t,x2t),
                     plot = dp2dF_plot_list[[sdmname]], 
          ...
          for (k2 in explo[-c(1:k)]) {
            for (k3 in explo[-c(1:k2)]) { #k3=4
              ...
              dfrq <-as.data.frame(na.omit(dfrq)) %>%
                rename(x1=!!x1t,x2=!!x2t,x3=!!x3t,y=!!yt)
              y <- dfrq$y <- jitter(dfrq$y)
              x1 <- dfrq$x1 <- jitter(dfrq$x1) 
              x2 <- dfrq$x2 <- jitter(dfrq$x2)
            ...
            for (k3 in explo[-c(1:k2)]) {
              for (k4 in explo[-c(1:k3)]) { #k4=4
                ...
                tryCatch({
                  modelq<-rq(y~x1*x2*x3*x4, tau=taus)
                  modelq_list[[sdmname]] <- modelq
                  xt<-paste(x1t,x2t,x3t,x4t,sep=paste(' ',
                                                      modelq$formula[[3]][[1]],' ')); 
                  xl<-paste(x1l,x2l,x3l,x4l,sep=paste(' ',
                                                      modelq$formula[[3]][[1]],' '))
                  modelq0<-rq(y ~ 1, tau=taus)
                  ...
                  for (t in 1:length(taus)){
                    smrq_t[[t]] <- as.data.frame(smrq[[t]][["coefficients"]]) %>%
                      mutate(Var=rownames(.)) %>% relocate(Var) %>%
                      remove_rownames(.) %>%
                      mutate(AIC=round(AIC(modelq)[t],1),
                             Rone=round((1 - modelq$rho[t]/modelq0$rho[t]),5)) %>%
                      mutate(tau=taus[t], taust=sprintf("tau= %s",tau),
        ...
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/RQ1/%s_%s_RQ1_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/RQ1/%s_%s_RQ1Resid_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- qqres_plot_list[grep(sprintf("2add_%g%g%g",sp,sa,rep),
                                     names(qqres_plot_list))]
        dp_plot <- lapply(dp_plot, function(x){
          x + theme(plot.title = element_blank(),
                    axis.title.y = element_blank(),
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/RQ2add/%s_%s_RQ2addResid_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        dp_plot <- qqres_plot_list[grep(sprintf("2int_%g%g%g",sp,sa,rep),
                                     names(qqres_plot_list))]
        dp_plot <- lapply(dp_plot, function(x){
          x + theme(plot.title = element_blank(),
                    axis.title.y = element_blank(),
        ...
        dp_plot <- annotate_figure(dp_plot,
                ...
                left = text_grob(yt, rot = 90)) +
        bgcolor("white")
        ggsave(sprintf("%s%s/%s/RQ2int/%s_%s_RQ2intResid_%s_%s.tiff",
                     wdgraph,speciesMP$SPCourt[sp],
                     analysis,etude,speciesMP$SPCourt[sp],
                     saison[sa,2],yt),
             plot = dp_plot,
        ...
        #         left = text_grob(yt, rot = 90)) +
        # bgcolor("white")
        # ggsave(sprintf("%s%s/%s/RQ1/%s_%s_RQ1_sm_%s_%s.tiff",
        #              wdgraph,speciesMP$SPCourt[sp],
        #              analysis,etude,speciesMP$SPCourt[sp],
        #              saison[sa,2],yt),
        #      plot = dp_plot,
        #      width = 18, height = 12, dpi=400)
        # 
        # dp_plot <- sm_plot_list[grep(sprintf("2add_%g%g%g",sp,sa,rep),
        #                              names(sm_plot_list))]
        # dp_plot <- lapply(dp_plot, function(x){
        #   x + theme(plot.title = element_blank(),
        #             axis.title.y = element_blank(),
        ...
        #         left = text_grob(yt, rot = 90)) +
        # bgcolor("white")
        # ggsave(sprintf("%s%s/%s/RQ2/%s_%s_RQ2add_sm_%s_%s.tiff",
        #              wdgraph,speciesMP$SPCourt[sp],
        #              analysis,etude,speciesMP$SPCourt[sp],
        #              saison[sa,2],yt),
        #      plot = dp_plot,
        #      width = 18, height = 12, dpi=400)
        # 
        # dp_plot <- sm_plot_list[grep(sprintf("2int_%g%g%g",sp,sa,rep),
        #                              names(sm_plot_list))]
        # dp_plot <- lapply(dp_plot, function(x){
        #   x + theme(plot.title = element_blank(),
        #             axis.title.y = element_blank(),
        ...
        #         left = text_grob(yt, rot = 90)) +
        # bgcolor("white")
        # ggsave(sprintf("%s%s/%s/RQ2/%s_%s_RQ2int_sm_%s_%s.tiff",
        #              wdgraph,speciesMP$SPCourt[sp],
        #              analysis,etude,speciesMP$SPCourt[sp],
        #              saison[sa,2],yt),
        #      plot = dp_plot,
  ...
  AIC_plot<-smrq_l %>% 
    filter(!str_detect(mode, "3var")) %>% 
    filter(!str_detect(mode, "4var")) %>%
    filter(!str_detect(mode, "add")) %>%
    group_by(tau) %>%
    ggplot(aes(x=reorder(predict,-AIC),
               y=AIC,
  ...
  rm(list=c("smrq","rqsup","rqinf","rqlim","RqMod_mat","RqMod_t",
