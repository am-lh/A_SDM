...
binderout <- sprintf(paste(wdwork,etude,"_binderout.xlsx", sep=""))
rdataout  <- sprintf(paste(wdwork,etude,"_RQ_BDD.RData", sep=""))
# if exists
# load(rdataout)
```
...
if (choixttt==1 | choixttt==12 | choixttt==13 | choixttt==123){
  ...
  modelq_rq_list <- list()
  smrq_t_rq_list <- list(); RqMod_rq_list <- list(); # rqResid_t_rq_list <- list()
  rqlim_t_rq_list <- list(); rqinf_t_rq_list <- list(); rqsup_t_rq_list <- list();
  for (sp in spe) { # sp=1
      ...
      for (rep in answ){ # rep=1
        for (k in explo) { # k=1 
          ...
          tryCatch({
            ...
            smrq_t_rq_list[[sdmname]] <- smrq_t <- smrq_t %>% map_dfr(., ~.)
            ...
            RqMod_t <- melt(RqMod_t,id.vars =c("x1"),variable.name = "tau",value.name = "RqMod") %>% 
              mutate(sdmname=sdmname,
                     taust=sprintf("tau= %s",tau),tau=as.numeric(as.character(tau)))
            RqMod_rq_list[[sdmname]] <- RqMod_t
            rqlim_t_rq_list[[sdmname]] <- rqlim_t
            rqsup_t_rq_list[[sdmname]] <- rqsup_t
            rqinf_t_rq_list[[sdmname]] <- rqinf_t
          ...
          for (k2 in explo[-c(1:k)]) { #k2=2
            ...
            tryCatch({
              ...
              # GRAPHIC PART ----
              # SUMMARY GRAPHS 
              png(file=sprintf("%s%s/%s/RQ2int/%s_%s_RQ2int_sm_%s_%s_%s_%s.tiff",
                               wdgraph,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],saison[sa,2],yt,x1t,x2t),
...
k_choosen<- which(pred_red$Var %in% c("flow_mxd")) # flow_mxd inunt sal_dtd temp_m mudrate_m tenfon_mxd
      ...
      for (k2 in k2_choosen) { # k2=k2_choosen 
        for (t in taus_choosen){ # t= taus_choosen
          for (rep in reponse_choosen){ # rep=reponse_choosen[1]
            ...
            modelq <- modelq_rq_list[[sdmname_chosen]]
            smrq_t <- smrq_t_rq_list[[sdmname_chosen]]
            RqMod_t <- RqMod_rq_list[[sdmname_chosen]]
            rqlim_t <- rqlim_t_rq_list[[sdmname_chosen]]
            rqsup_t <- rqsup_t_rq_list[[sdmname_chosen]]
            rqinf_t <- rqinf_t_rq_list[[sdmname_chosen]]
            df <- CSLN_Mars[which(CSLN_Mars$SPCourt == speciesMP$SPCourt[sp]),]
            yt = reponse[rep,1] ; yl = sprintf("%s (%s)",reponse[rep,2],reponse[rep,3])
            zt = "Zone"
            x1t = sprintf("%s%s",pred_red[k,1],saison[sa,1]); x1l = sprintf("%s%s (%s)",pred_red[k,2],saison[sa,1],pred_red[k,3])
            ...
            ggsave(sprintf("%s%s/%s/%s_%s_RQ2int2d_%s_%s_%s_%s_%s.tiff",
          ...
          ggsave(sprintf("%s%s/%s/%s_%s_RQ2int2d_%s_%s_%s_%s.tiff",
                         wdgraph,speciesMP$SPCourt[sp],analysis,etude,speciesMP$SPCourt[sp],
...
if (choixttt==3 | choixttt==13 | choixttt==23 | choixttt==123){
          ...
          for (t in taus_choosen){ # t= taus_choosen
            for (rep in reponse_choosen){ # rep=reponse_choosen[1]
              ...
              for (ann in 1:length(anMars)) { # ann=1
                  ...
                  ggsave(sprintf("%s%s/%s/Map Detail/%s_%s_%s_%s_%s_%s.tiff",
              ...
              ggsave(sprintf("%s%s/%s/%s_%s_%s_%s_%s.tiff",
                 wdgraphEx,speciesMP[sp,1],analysis,
  ...
  st_write(Mars_SDM, sprintf("%sLayers made/SDM_NEO_RQ_%s.shp",
                             wdGIS,speciesMP$SPCourt[sp]),append=FALSE)
