...
```{r}
#| label: externdata
binderin <- sprintf("%sCSLN_BDD.xlsx", wdres)
...
# save(ad,CSLN_adonis,CSLN_pairado,file = paste(wdmat,"CSLN_Zone_Ado.RData", sep=""))

# load(paste(wdmat,"CSLN_Zone_Ado.RData", sep=""))
...
ggd1 <- ggd1 %>% set("branches_k_color", k=CSLN_simprof$numgroups,
                     value=colDarj(CSLN_simprof$numgroups)) %>% #
          set("branches_lwd", .8) %>%
...
print(ggd1)
ggsave(paste(wdgraph,"CSLN_Simprof_Zone",".png",sep=""), 
       plot = ggd1, width = 12, height = 8, dpi=400)
...
# save.image(file = paste(wdmat,"CSLN_BDD_Zone",".RData", sep=""))
...
print(bp)
ggsave(paste(wdgraph,"nMDS Macrofaune by Zone",".png",sep=""), 
       plot = bp, width = 12, height = 8, dpi=400)
