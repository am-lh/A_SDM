  ...
  rq_nLin <- function(sdmod,df,biolo,typ,taus) {
    ...
    modt<-map(as.list(taus), \(taut) { # taut=taus[[4]]
      tryCatch({
        ...
        boot_res <- boot(dfrq, calc_rq, R = 3, 
...
my_func <- function(data, indices, taut) {
  d <- data[indices,]
...
# create a function to perform the bootstrap and return a table of results
bootstrap_table <- function(data, taut, bin, n_bootstraps) {
    # data=rq_mod_tmp; taut=0.95; bin=9; n_bootstraps=10
  subset_data <- data[data$tau == taut & 
                      data$points_bin == bin,]

  bootstrap_results <- boot(subset_data, my_func, R = n_bootstraps, taut=taut)
...
rq_po_plot <- function(data, metas) {
...
rq_po_plot_boot<- function(rqMod) {
     # rqMod=rq_Mod_sel[[1]][[1]]
  # create adequate data table for bootstrap
  ...
  as.list(expand_grid(
    ...
    transpose %>%
    map(~bootstrap_table(data = rq_mod_tmp, 
                         taut = .x$taut, bin = .x$bins, 
                         n_bootstraps = 1000)) %>% 
    bind_rows() %>%
    mutate(tau=as.factor(tau)) %>% 
    rq_po_plot(.,metas=rqMod$meta)
}
...
st_rq_Mod_map<-function(rqmodmars,rqMod,tauchoice){
  ...
  rqmodmars <- rqmodmars %>% 
    ...
    mutate(across(any_of(c("bathy","tenfon_mxd")), ~replace_outliers(.,1.5))) %>%
    mutate(Zone=fct_drop(.$Zone)) %>% 
    mutate(across(c(Zone,Period),factor))
  
  biorange<-max(
    max(replace_outliers(rqmodmars %>% select(sprintf("t%.3f",tauchoice))), na.rm=TRUE),
    max(replace_outliers(rqMod$rqlim_t$y), na.rm=TRUE)) %>% 
    ceiling(.)
  ...
  tmp<-rqMod$rqlim_t %>%
    ...
    summarise(Observed = quantile(y, unique(tauchoice), na.rm=TRUE),
              n=n())
  # add here the count of data to make the size of the point linked to it??
  ...
  descr$mod <-rqmodmars %>% 
    ...
    geom_density_ridges(aes(y = Period , fill=Period, 
                        ...
                        alpha=0.25, size = .2) + #stat="density", 
...
pl_rq_mars_sel_st_board<-map_depth(pl_rq_mars_sel_st, 2,
            ~.x$mod) %>% 
  map(., ~wrap_plots(.x) &
    plot_layout(ncol=1) &
        theme(legend.position="none",
...
max_map<-map(rq_Mod_mars_sel, ~{ # nlrqdata$
  map_dbl(.x, ~max(
    replace_outliers(.x[[sprintf("t%.3f",tauchoice)]]),na.rm=TRUE) ) %>% 
