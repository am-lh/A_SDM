...
fauna_file <- sprintf("%sCSLN_Biology_source.xlsx",wdsource)
...
BioturbP <- as.data.frame(read_excel(
  sprintf("%s/Data/Faune/Bioturbation_Potential.xlsx",wdproj),
  sheet = "Bioturbation_Potential", na = ""))
# summary(CSLN_raw)
...
mars_file <- sprintf("%sES_Ncf_BDD.xlsx",wdres)
...
ES_Areas <- sprintf("%sLayers made/ES_Areas_WGS.shp",wdGIS)
ES_Areas <- st_read(ES_Areas,quiet=TRUE,crs=4326) %>%
  st_transform(2154) %>%
  filter(Zone!="NA")

Mars_csv <- sprintf("%sES_Mars_Maps_sh.csv",wdres)
Mars_csv <- read.csv(Mars_csv)
Mars_csv[Mars_csv == "NaN"] <- NA
Mars_csv <- Mars_csv %>% 
  filter(!is.na(Lat) & !is.na(Lon) & !is.na(flow_m)) #%>%
#  filter(Lat!="NaN" | Lon!="NaN")

Mars_dat_sf <- sprintf("%sLayers made/ES_Maille_nc.shp",wdGIS)
Mars_dat_sf <- st_read(Mars_dat_sf,quiet=TRUE,crs=4326) %>%
  st_transform(2154) %>%
...
speciesList <- BioturbP %>% 
  select(SPCourt,ScientificName_accepted) %>% 
...
reponse <- data.frame(
  rvar=c("Biomass_gAFDWm2","Density_indm2","MSRtot"),
...
tidelevels<-data.frame(
  level=c("upper intertidal", "medium intertidal", "lower intertidal", "subtidal"),
  descr=c("[0-25%[ immersion time","[25-75%[ immersion time","[75-100%[ immersion time","100% immersion time")) %>% 
  mutate(level=ordered(level,c("upper intertidal", "medium intertidal", "lower intertidal", "subtidal")))
...
Mars_csv$Tidal_level[Mars_csv$inunt>=0 & Mars_csv$inunt<0.25] <- tidelevels$level[1]
Mars_csv$Tidal_level[Mars_csv$inunt>=0.25 & Mars_csv$inunt<0.75] <- tidelevels$level[2]
Mars_csv$Tidal_level[Mars_csv$inunt>=0.75 & Mars_csv$inunt<1] <- tidelevels$level[3]
Mars_csv$Tidal_level[Mars_csv$inunt==1] <- tidelevels$level[4]
...
Mars_dat_sf$Tidal_level[Mars_dat_sf$inunt>=0 & Mars_dat_sf$inunt<0.25] <- tidelevels$level[1]
Mars_dat_sf$Tidal_level[Mars_dat_sf$inunt>=0.25 & Mars_dat_sf$inunt<0.75] <- tidelevels$level[2]
Mars_dat_sf$Tidal_level[Mars_dat_sf$inunt>=0.75 & Mars_dat_sf$inunt<1] <- tidelevels$level[3]
Mars_dat_sf$Tidal_level[Mars_dat_sf$inunt==1] <- tidelevels$level[4]
...
CSLN <-  CSLN %>% 
  left_join(BioturbP %>% 
              select(AphiaID_accepted,Mi,Ri), by=c("AphiaID_accepted"))
...
CSLN_Mars$MSR_mW <- NA
setwd(paste0(wdscript,"MSR")) # when the package is done, with repertory in it, no more needed
source(wdmsr)
...
# save.image(file = paste0(wdmat,"CSLN_Mars_BDD.RData"))
# load(file = "/Matrices/CSLN_Mars_BDD.RData")
...
model_types<- data.frame(rbind(
  c(analysis="RQ Linear",code="RQ2add",bdd="rqdata"),
  c(analysis="RQ Linear",code="RQ2int",bdd="rqdata"),
  c(analysis="RQ Nonlinear",code="RQ2nli",bdd="nlrqdata"),
  c(analysis="RQ BSpline",code="RQ2bsp",bdd="rqbsdata")))

reponse<-reponse %>%
  filter(rvar %in% c("Biomass_gAFDWm2","Density_indm2")) %>%
  rowid_to_column("repi") %>% 
  mutate(compl=sprintf("%s (%s)",rdescr,runit))
...
sdm_comb<- tidyr::expand_grid(model_types %>% rowid_to_column("type"),
                         ...
                         saison %>% 
                           ...
                           rowid_to_column("sai"),
                         pred_red_comb_t) %>%
  unite("type",c(sp,sai,repi,id),sep="", remove = FALSE) %>% 
...
# Run of all sequence after that (ca pas l'air demarcher comme ça avec les qmd...)
```{r}

# source("Scripts/2.0_Datasets_visualisation.qmd")
# source("Scripts/3.1_RegQuant_lineaire.qmd")
# source("Scripts/3.2_RegQuant_non_lineaire.qmd")
# source("Scripts/3.3_RegQuant_BSpline.qmd")
