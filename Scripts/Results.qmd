---
title: "A modelling approach for predicting species distribution as function of physical drivers in Seine estuary by applying an Optimal Ecological Niche model : application to Cerastoderma edule population"
author: "Lehuen Amélie*, Grasso Florent, Dancie Chloé, Orvain Francis"
...
## Description of biological data set
...
The selected predictors were observed with the same period and areas factors than the biological data (@sec-hms_struc, the map of their value was represented in the estuary @fig-mars_map). On a global point of view, all factors show significant differences between areas and periods:
...
The four scenarios were treated with the two selected model types: either linear with interaction, or non-linear with bifactorial gaussian equation with the biomass as biological response (@sec-sdm_board, predict/observed plot in  @sec-qq_sdm, SDM-NEOs with the distribution of HMS data in @sec-sdm_real, RQ2nli SDM-NEOs were represented in 3D graph in @sec-sdm_3d).
...
#| include: true
#| results: asis
#| fig-cap: "Scenarios computed with linear model (top, numbered 1) and nonlinear with gaussian equation (bottom numbered 2), the observed biological data under the model were represented by an isometric curve, over by red stars. Each couple had its own scale to ensure visibility."
#| fig-width: 16
#| fig-height: 8
...
The non-linear model was globally more performant than the linear, it was the only one geographically analysed. Each SDM-NEO for RQ2nli model was applied on the HMS model web over the estuary, for each period (@sec-sdm_map). Density plots were made for each SDM-NEO, focusing on mudflats to ease the comparisons (@sec-sdm_dens).
...
#### Application density plots

::: panel-tabset

##### Biomass {#sec-sdm_dens}
...
::: column-page-right
The four models result on the estuary regarding Areas and Periods, with the two calculation modes, for both response (biomass: @sec-sdm_st_det_b, density: @sec-sdm_st_det_d)
