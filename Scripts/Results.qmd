...
All Seine Bay MARS 3D model was calculated in a mesh of `r formatC(700*112, format="f", big.mark=",", digits=0)` cells (700 by 112), focused on the Seine Estuary with a mesh of `r marsweb` cells (`r marsxy`), where the cells were 100m long in average, with the coordinates of Longitude `r marslonlat`. Each area defined regroup several cells (`r nbmailzone`).

HMS model cells corresponding to the stations localisation in the biological dataset were at total `r marsmaillenb` with the distribution in areas as `r marsmaillezone`. As for the distinction in the position in the tidal area: `r marsmailledet`).
:::
...
A correlation study was conducted on all HMS variables to select the more pertinent factors and avoid autocorrelation between factors (@fig-mars_corr). All environment factors were the result of the annual mean of model variables.
...
-   **Salinity daily range** \[`r factors[5,3]`\] (min-max of the day) was preferred to salinity \[`r factors[4,3]`\] to emphasize more the tidal influence (Corr = `r dfcornew %>% filter(variable %in% "sal_dtd") %>% select(sal_m) %>% pull()`).
...
-   **Mud content** \[`r factors[14,3]`\] (particles \<63µm) was chosen over sediment total conc \[`r factors[13,3]`\] to link with other references (Corr = `r dfcornew %>% filter(variable %in% "mudrate_m") %>% select(Csed_m) %>% pull()`).
...
plot_rq<-map2(rqdata$pl_rq_po_sel,sdm_comb_sel_l,
            ~ map2(.x,.y[names(.x)],
              ~.x + labs(title=.y$analysis) ) ) #; plot_rq$Biomass$RQ2int_11112
plot_nlrq<-map2(nlrqdata$pl_rq_po_sel,sdm_comb_sel_l,
            ~ map2(.x,.y[names(.x)],
              ~.x + labs(title=.y$analysis)) )
plot_rqbs<-map2(rqbsdata$pl_rq_po_sel,sdm_comb_sel_l,
            ~ map2(.x,.y[names(.x)],
              ~.x + labs(title=.y$analysis)) ) 

pl_tmp<- pmap(list(plot_rq,plot_nlrq,plot_rqbs,sdm_comb_sel_l) ,
...
pl_board_rq_nlrq <- 
  pmap(list(plot_rq,plot_nlrq,reponse_l),
    ~ map2(..1,..2,
      ...
      labs(fill=..3$Unit) & #
      plot_annotation(title = sprintf("SDM-NEO based on %s (tau=%g)",..3$compl,taus[taut]),
                      tag_levels = c('A', '1')) & 
      theme()
...
pl_rq_suit_idx_nlrq_brd <- map2(nlrqdata$pl_rq_suit_idx,reponse_l,
  ~{wrap_plots(.x, nrow=1,guides = 'collect') &
    ...
    plot_annotation(title = sprintf("SDM-NEO %s based on %s (tau=%g)",
                                    model_types$analysis[3],.y$compl,tauchoice),
                    tag_levels = c('A')) & 
...
ggsave(pl_tmp$Biomass, width = 12, height = 12, dpi=600, filename= 
...
pl_tmp$Density<-pl_tmp$Density &
  xlim(c(0,3500))

pl_tmp$Density

ggsave(pl_tmp$Density, width = 12, height = 12, dpi=600, filename= 
