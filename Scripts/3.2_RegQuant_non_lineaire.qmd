   ...
   rqMod$rqlim_t %>% 
    ...
    ggplot(aes(x=Observed,y=Predict,col=tau,fill=tau,shape=tau)) +
      ...
      stat_cor(aes(label=after_stat(r.label)),
               label.x.npc = .6,label.y.npc=.3,show.legend = FALSE) +
...
# library(boot)
# set.seed(1)
# st_quant<-function(data,indices) {
#     data[indices,] %>% 
#     mutate(taustx=as.character(tau)) %>% 
#     nest(data=-taustx) %>% 
#     mutate(data=map(data, ~{.x %>% 
...
# }
# library(modelr)
# out <- rq_Mod_sel$Biomass$RQ2nli_11112$rqlim_t %>%
#           mutate(taustx=as.character(tau)) %>% 
#           group_by(taustx) %>% 
#           nest %>% 
#           mutate(data=map(data, ~{.x %>% 
...
#                   mutate(.,n=n())
#             })) %>% 
#          mutate(boot = map(data, ~ modelr::bootstrap(.x, n = 100, id = 'y') #%>%
#                                  # pull('strap')# %>% 
#                                  # map_dbl(~ as_tibble(.x) %>% 
#                                  #          pull('y') %>%
#                                  #          median) 
...
# 
# 
# results <- boot(data=rq_Mod_sel$Biomass$RQ2nli_11112$rqlim_t,
#                               statistic=st_quant,
#                               R=1000)
# 
...
st_rq_Mod_map<-function(rqmodmars,rqMod,tauchoice){
  ...
  biorange<-max(
    max(replace_outliers(rqmodmars %>% select(sprintf("t%.3f",tauchoice)),3), na.rm=TRUE),
