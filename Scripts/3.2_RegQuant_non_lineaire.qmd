...
rq_po_plot <- function(rqMod) { # without bootstrap
    # rqMod=rq_Mod_sel[[1]][[1]]
...
rq_po_plot <- function(data, metas) { # for the bootstrap
...
calc_boot <- function(data, indices, taut) {
  d <- data[indices,]
...
bootstrap_table <- function(data, taut, bin, n_bootstraps) {
  ...
  bootstrap_results <- boot(subset_data, calc_boot, R = n_bootstraps, taut=taut)
