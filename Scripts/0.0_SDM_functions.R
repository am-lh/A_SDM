...
conflict_prefer_all("GGally",losers="terra",quiet = TRUE)
conflict_prefer_all("patchwork",losers="terra",quiet = TRUE)
...
mystars <- function(p) { ifelse(p < .0001, "****", 
                                ifelse(p < .001, "***", 
                                       ifelse(p < .01, "**", 
                                              ifelse(p < .05, "*",
                                                     ifelse(p < .1, "·", ""))))) }
...
f.pl_var_map_gif<-function(df,var){
  # df=Mars_tmp
  # var=pred_red %>% transpose %>% setNames(.,pred_red$Var); var=var[[1]]
...
f.pl_rq_Mod_map_gif<-function(rqmodmars,sdmname,tauchoice){
  # rqmodmars=rq_Mod_mars_sel[[2]][[1]]
  # sdmname=names(rq_Mod_mars_sel[[2]])[1]
